...
## C19 — Architecture spine altitude 与 divergence closure

每次创建或更新 spine，先记录它的 altitude 和明确的下一级单位：`initiative→Epic`、`Epic→Story` 或 `feature→unit`。Spine 只关闭这一层需要共同遵守的 durable invariants；范围若不符合这三类，必须先明确等价 parent/lower-unit identity，不能让“system architecture”掩盖实际约束高度。

对每个候选项同时执行两个 predicate：

1. 两个下一级单元独立实现时，是否可能选择不兼容答案；
2. 该答案是否非显然且存在真实 trade-off。

只有两项都为真才升级为 durable Architecture decision。任一为假时，把可从 compliant code 读取的现状保留为 minimal seed/context，或把单一 Story/下一级单元的私有细节留给其本地实现；不得把 stack tree、full schema 或单 Story 私有设计提升为长期 invariant。

Parent spine 的 invariants 以原 stable ID、rule、source identity 与适用边界进入 inherited ledger，并作为 read-only constraints。Local spine 不得重编号、弱化、覆盖或重新决定；任何冲突都保留双方原始信号，升级到 parent Architecture owner，设置 `currentNode=architecture_parent_conflict` 和恢复用 `firstUnfinishedAction`，并阻止 F6。

每条 durable decision 记录 stable ID、`Binds`、`Prevents`、`Rule`、source 与 decision owner；`Binds` 命名受约束的下一级单位，`Prevents` 命名不兼容分叉。它复用 T3 的唯一 owner/lifecycle/data/error responsibility，不创建第二 owner ledger。

对每个适用结构维度记录 `DECIDED / DEFERRED / OPEN / N/A`：paradigm、boundary/dependency、state mutation、owner/lifecycle、shared data responsibility、error/invariant 与 operational envelope。`DEFERRED` 必须证明该项不会导致当前下一级单元分叉，并记录 owner 与可观察 revisit condition；`N/A` 必须有 source-bound 理由。任何 F5-owned 适用 `OPEN`、无证明的 `DEFERRED` 或 inherited conflict 都是 `phaseBlocker`，不得进入 F6。
...
- spine altitude/下一级单位未明确，或候选 decision 未通过双条件 divergence test；
- inherited invariant 被弱化/覆盖，或 parent conflict 未升级；
- 适用结构维度为 `OPEN`，或 `DEFERRED` 无 non-divergence proof、owner/revisit condition；
...
产出 altitude/lower-unit identity、read-only inherited ledger、minimal seed/context、accepted Architecture decisions、dimension closure、spine、responsibility/owner/lifecycle maps、production modification set、data/identity/flow/error/security/operation contracts、feasibility evidence identities、rejected routes、limitations 与 downstream ripple。

成功时 altitude 与下一级单位明确，每条 durable decision 通过双条件 divergence test，parent invariants只读继承，所有适用维度均为 `DECIDED`、有证明的 `DEFERRED` 或有理由的 `N/A` 且无 owned `OPEN`；每个 load-bearing mechanism 对 exact version 可证明；每个责任维度只有一个 primary owner；Writer 不需新增 core ownership；Architecture 可稳定跨 Story，并能由 F6/F7 建立独立 evidence。然后记录 `currentNode=epic_story_dag` 或最早缺口，并 handoff F6/F7。

...
先固定 intent：`setup`（尚无 context）、`refresh`（重新验证已有 context）、`record`（记录一次有证据的 agent mistake）或 `audit`（逐行复核和收缩）。四种 intent 共享且只共享以下 mandatory pre-write gate，不各自建立 write authority：

```text
proposedMutation = NONE | MANAGED_REGION_WRITE
writeGate:
  targetIdentity: status + exact repository/worktree + exact target file/managed region + evidence + recovery
  hostWritePermission: status + current host/user + exact-target write/commit permission + evidence/scope + recovery
  previewApproval: status + complete target-bound proposed bytes/delta identity + explicit user approval identity + recovery
```

当 `proposedMutation != NONE` 时，三个字段必须分别为 `PROVEN/PASS` 才能写；任一字段为 `UNKNOWN`、`CONFLICT` 或 `FAIL` 都返回 `PROJECT_CONTEXT_BLOCKED`、`writeOrMutation=false`，并逐字段给出最小恢复动作。`hostWritePermission` 不得从 target 存在、用户是 decision owner、请求“现在写”、能生成 preview、用户批准文本、branch 存在或先前其他 target 的权限推断；`previewApproval` 不能替代 host permission，host permission 也不能替代用户对 exact preview 的批准。若 `proposedMutation=NONE`，仍逐字段报告：target 与 permission 保留实际状态，`previewApproval=NOT_APPLICABLE_NO_MUTATION`；no-op 不制造 write authority。

然后：

1. 分别解析 `targetIdentity` 并验证该 exact target 的 `hostWritePermission`。只有 workspace/build manifest 或独立 build unit 能证明 child scope；sibling repository 不是 child。多个 target、不可提交 target 或 nested scope 不明确时，先展示证据并停止 mutation。
...
当 `setup / refresh / record / audit` 提出、确认或解决一个会改变行为的 `HUMAN_ONLY` policy、contradiction 或 scoped supersession 时，必须在进入 exact-write approval 前明确输出：

- `directRipple`：该决定直接改变的已证明行为、流程或约束；
- `affectedConsumers`：会直接读取或执行该决定的 downstream consumers。

两者只能从当前 case 的已证明 scope 与 evidence 导出；未知项保持 `UNKNOWN`，不得猜测或用通用清单补全。它们是现有 decision/preview 的输出义务，不是第四个 write gate，也不创建第二 authority、owner、schema family、helper、wrapper、manager、fallback 或 runtime。缺少承重 `directRipple` 或 `affectedConsumers` 时，保持当前 `currentNode`，要求 final decision owner 补齐，并返回 `PROJECT_CONTEXT_BLOCKED`、`writeOrMutation=false`；推荐不是决定，不得进入写入。
...
5. 输出 root 与每个 accepted child 的完整最小 context preview，逐条列 source、verification identity、保留/排除理由、contradiction 与 proposed target，并把完整 target-bound proposed bytes/delta identity 绑定到 `previewApproval`。只有同一 `writeGate` 三字段对该 exact mutation 全部为 `PROVEN/PASS` 才能写；获准写时只改批准的 managed region，并保持其外 bytes 不变；永不 commit。

`setup / refresh / record / audit` 的成功、blocked 与 recovery 输出都必须携带 `proposedMutation`、同一个三字段 `writeGate` 和 `writeOrMutation`，并保留原有 `verified context preview/findings` 或 exact context delta。只有真实写入才声明 `writeOrMutation=true`；preview、findings、approval 或 no-op 都不得伪报 mutation。每个 path-backed claim 可解析，human-only rule 不伪装成 repo fact，nested rule 只约束其目录。contradiction、stale provenance、missing path 或任一 writeGate gap 返回 `PROJECT_CONTEXT_BLOCKED`，保留 competing evidence，并在对应字段报告最小恢复条件。

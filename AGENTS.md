...
When a complete prompt gives a local artifact path, use that exact literal path; do not derive a filename from the Task, Role, Attempt, candidate label, or validation label. A failed read is a recoverable path/input error: reread the current prompt and retry the stated path. If that exact path is objectively absent, report that fact without inventing or silently substituting another artifact.

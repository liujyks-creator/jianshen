...
- Validation profile：<风险等级与准确的比例检查>
...
冷启动与独立性：
1. 完整读取一次适用技能、pinned review base 中 accepted AGENTS.md、本完整 accepted 模板，以及仅与当前 candidate 直接相关的 Story/decision/testing/evidence 来源；记录准确来源身份。
...
同一对话自动上下文压缩后：
- 使用系统摘要继续，核验 base/candidate、已完成 Review 轴、evidence identity 与首个未完成轴。
- 不因压缩重复完整读取、已完成验证、构建或设备步骤。
- 不输出部分 findings；完成当前节点全部适用 Review 轴后一次性返回。
...
- 在报告 findings 前完成当前节点 scope、acceptance、quality、evidence、Git 与 protected-state 全部适用轴。

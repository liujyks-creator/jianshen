...
E11.3 后，首版训练执行页不显示心率位、不显示未获取心率占位，也不提供手动心率输入。E16 Band 9 心率广播 retest 已提供 BLE HRS 正向证据；E16-1 已实现 debug-only BLE HRS adapter spike 和纯 payload parser；E16-2 已实现 production-capable provider / permission / lifecycle 地基，但仍不允许社区 UI 或生产 shell 恢复心率卡片。E16-3 顶部 pill 初版已被后续讨论取代；E16-3a 已完成 App 内可拖动浮动心率胶囊 HTML 修订；E16-4 已明确心率必须默认关闭，并通过设置页显式 opt-in、权限说明、设备选择、隐私和非医疗文案进入。该胶囊只能作为 TrainFlow app 内 overlay，不使用系统级悬浮窗权限，不参与训练页布局，不得遮挡当前动作、主时间、下一步、组目标、confirm-record、输入框、底部主按钮或系统安全区。社区 skin 若未来展示心率，必须沿用连接 / 数据状态与“区间 + bpm”状态的拆分，不得恢复 `-- bpm`、`未获取心率`、手动输入或旧平均心率趋势；也不得绕过设置页 opt-in、在 app 启动 / 训练页进入 / 训练开始时请求 BLE 权限、静默扫描、后台无限扫描或保存 GATT / SDK model。后续如果重新评估健康设备或心率展示，必须先更新产品决策和权限 / 数据边界，并按 settings / opt-in、permission flow、device picker、capsule UI、state mapping、stale policy、recording model 和 analysis 分 story 实现；未来健康数据仍不能挤压当前动作、主时间、下一步和主控制。
...
- 当前首版是否没有心率卡片、未获取心率占位或手动心率输入；未来浮动心率胶囊不得高于当前动作、倒计时、组目标和主按钮，也不得最终停在主按钮、底部导航、confirm-record 控件、输入框、键盘区域、状态栏或手势导航上方。
- 未来心率 UI 是否默认关闭，并且只能经设置页显式开启；BLE 权限是否只在用户主动开启 / 选择设备 / 重新扫描后请求，且不使用系统 overlay 权限、不静默扫描、不后台无限扫描。
- 未来设备选择是否只保存 identifier / display name，关闭后是否不扫描、不连接、不记录，并提供清除已保存设备入口。

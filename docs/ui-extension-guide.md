...
E11.3 后，首版训练执行页不显示心率位、不显示未获取心率占位，也不提供手动心率输入。E16 Band 9 心率广播 retest 已提供 BLE HRS 正向证据；E16-1 已实现 debug-only BLE HRS adapter spike 和纯 payload parser；E16-2 已实现 production-capable provider / permission / lifecycle 地基，但仍不允许社区 UI 或生产 shell 恢复心率卡片。E16-3 顶部 pill 初版已被后续讨论取代；当前未来方向是 E16-3a App 内可拖动浮动心率胶囊。该胶囊只能作为 TrainFlow app 内 overlay，不使用系统级悬浮窗权限，不参与训练页布局，不得遮挡当前动作、主时间、下一步、组目标、confirm-record、输入框、底部主按钮或系统安全区。社区 skin 若未来展示心率，必须沿用连接 / 数据状态与“区间 + bpm”状态的拆分，不得恢复 `-- bpm`、`未获取心率`、手动输入或旧平均心率趋势。后续如果重新评估健康设备或心率展示，必须先更新产品决策和权限 / 数据边界，并先做 HTML 视觉方案 / 高保真案例评审，再进入 Android UI 实现；未来健康数据仍不能挤压当前动作、主时间、下一步和主控制。

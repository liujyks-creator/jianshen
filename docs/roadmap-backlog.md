...
- E15-1b planned：在力量计划编辑页暴露计划级本组计时模式设置，让用户可以显式把已保存力量计划改为 `manual_start` 或 `auto_after_rest` 并保存。训练偏好继续只作为新建 / 编辑计划默认值，不运行时覆盖旧计划；执行引擎继续消费 `StrengthExerciseBlock.setTimerMode`。该 story 不改 engine 语义、Room schema、声音系统、TimerDial、记录、心率或设备接入。

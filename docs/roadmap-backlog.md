...
33. E16-10b：Heart-rate freshness / foreground reconnect implementation。已拆分为独立 implementation stories；umbrella 当前为 in progress。E16-10b-1 policy/core 已 reviewed / merged；E16-10b-2 已实现并等待独立 review，后续 UI mapping 与验证阶段继续锁定。必须遵守 E16-10a 已批准参数，禁止自动扫描、后台连接、自动换 target，以及权限重新授予或 retry 耗尽后的自动恢复；不得进入记录或分析。（In progress）
   - E16-10b-1：Heart-rate freshness policy core。新增纯 Kotlin monotonic timeline / policy / stable reason codes，覆盖 waiting `<15s` / stale `15s..<30s` / first-sample silence `>=30s`，live `<10s` / stale `10s..<30s` / notify silence `>=30s`；valid bpm 重置 freshness，parse failure 不刷新，明确 disconnect 与技术失败事实分离，retry exhausted 不改事实，异常时间 fail closed。Story tip `09d17616f213c1df7905e46662f4a195345fdd9a` 已通过 merge commit `5cdee7ce1bd7a2b0f76f83adf069179a547fd16c` 合入 `main`；production消费已由E16-10b-2实现并等待review。（Reviewed / merged）
   - E16-10b-2：Foreground reconnect controller。单一纯 Kotlin controller 已把 E16-10b-1 policy 接入 production provider，使用 monotonic clock与可取消 scheduler覆盖 waiting/live/stale/offline/technical error、2 / 5 / 10 秒最多三次 direct retry、10 秒 connect-stage watchdog、GATT/target/attempt guard、intentional close、scan coexistence和 lifecycle cancellation；deterministic tests与固定 AVD non-BLE smoke已通过。AVD 不证明真实 Band 9 reconnect，且 b3 最终 mapper/settings copy未实现。（Implemented / Needs review）
...
E11.1 Heart-rate source boundary / unavailable state refinement 已实现并保留为底层边界；E16-10b umbrella为in progress。E16-10b-1 pure policy core已 reviewed / merged；E16-10b-2 foreground reconnect controller已 implemented / needs review，production provider已消费policy并完成deterministic tests与AVD non-BLE smoke，但真实Band 9 reconnect尚未验收。E16-10b-3 UI mapper/settings copy与E16-10b-4 verification / closeout locked / not started；E16-11 / E16-12 not started。
...
```text
E15 系列已收口。E16-10b umbrella现为in progress：E16-10b-1已 reviewed / merged，E16-10b-2 foreground reconnect controller已 implemented / needs review；E16-10b-3 / b4 locked / not started，E16-11 / E16-12 not started。下一步只能执行E16-10b-2独立Code Review gate，不得启动后续Story。
```

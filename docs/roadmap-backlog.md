...
32. E16-10a：Heart-rate freshness / offline / reconnect policy planning。主管理已批准方案 B、10 秒 live stale、15 秒 waiting stale、30 秒 notify-abnormal、2/5/10 秒退避、最多 3 次、每次 10 秒 watchdog。有限 direct GATT reconnect 只对当前前台进程、本次已 live bpm 的同一 runtime target 生效，不 scan、不自动换 target；冷启动、回到前台、蓝牙恢复和 retry 耗尽后均不自动 scan/connect。设置页需提供 `停止连接` 取消队列并 suppress 本前台周期自动恢复；retry exhausted 沿用明确断开 `离线`、技术失败 `连接异常`，可显示手动恢复文案。该 docs-only story 不实现 timer/retry/backoff、UI 操作或 BLE 行为，不新增 schema 或记录。（Implemented / Needs review; policy approved）
33. E16-10b：Heart-rate freshness / foreground reconnect implementation。尚未开始；负责可测试 policy/core、foreground reconnect controller、UI mapper/settings copy（含 `停止连接`）以及 focused tests / Android checks / AVD 与真实 Band 9 验证收口。必须遵守 E16-10a 已批准参数，禁止自动扫描、后台连接、自动换 target 和 retry 耗尽后的自动恢复；不得进入记录或分析。（Not started）

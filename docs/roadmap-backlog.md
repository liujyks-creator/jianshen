...
32. E16-10a：Heart-rate freshness / offline / reconnect policy planning。已新增 `docs/testing/e16-10-heart-rate-freshness-reconnect-policy.md`，列出全手动恢复、前台有限 direct GATT reconnect（推荐）与自动重新扫描（不推荐）方向，并将 10 秒 live stale、15 秒 waiting stale、30 秒 notify-abnormal、2/5/10 秒退避、最多 3 次、每次 10 秒 watchdog 作为**待主管理对话确认**的候选值。推荐方向只对当前前台进程已 live 的同一 runtime target 生效，不 scan、不自动换 target；冷启动、回前台、蓝牙恢复和 retry 耗尽后均不自动 scan/connect。该 docs-only story 不实现 timer/retry/backoff，不新增 schema 或记录，不进入 E16-11 / E16-12。（Needs review）

...
5. **E17-4 Implementation readiness**：主文档为 `docs/planning/e17-4-heart-rate-implementation-readiness.md`。真实代码盘点已校正 627 行旧 provider、空 Application、Compose owner、三个 Route writer、Manifest / Service 缺口和胶囊适配范围；默认不新增 BLE operations seam，freshness 采用 E17-5 前台测量 -> E17-9 FGS 锁屏 / 后台补测 -> final lock 的无循环门禁。候选结论为 `ready for implementation review`。（Implemented / needs review；production locked）
6. **E17-5 Facts / freshness / presentation core**：唯一风险轴为三层事实与 public / capsule mapping、monotonic freshness 和旧 bpm 隔离；先完成前台 Band 9 测量 M0，只允许 provisional internal threshold，不发布最终 Live 合同。（Prerequisite-locked）
7. **E17-6 Deterministic Android BLE runtime owner**：唯一风险轴为 manual scan / connect、callback identity、generation / attempt、早到 callback、具体 Android BLE 调用与幂等 cleanup；replace 旧 provider，不创建通用 GATT wrapper。（Prerequisite-locked）
8. **E17-7 Application / settings / capsule production wiring**：唯一风险轴为 Application composition root、explicit opt-in / permission / picker、cold start 与冻结胶囊外部适配；完成 production 基本 Band 9 gate。（Prerequisite-locked）
...
10. **E17-9 Connected-device FGS / ID `7200` handoff**：唯一风险轴为 ordinary / FGS 单一 writer升降级、permission 分流、terminal ownership、Service / process death / Task Manager cleanup；完成锁屏 / 临时后台 Band 9 gate并在 merge 前锁定 final freshness。（Prerequisite-locked）
11. **E17-10 Integrated AVD / Band 9 production acceptance**：唯一风险轴为全序列回归、权限 / lifecycle、单一通知、process recreation 与两个真机 gate 的最终 evidence；不增加新架构或产品能力。（Prerequisite-locked）

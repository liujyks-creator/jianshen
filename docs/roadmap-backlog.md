...
2. **E17-1 Band 9 与标准 HRS 重新复验**：重新验证广播、UUID、CCCD、notify、payload、identifier 和 Huawei Health 互斥；真机事实不继承旧 E16 acceptance。启动资格由下方 closeout 条件式门禁决定，且它是设备/协议复验而不是 production implementation。（Planned / prerequisite-gated）
...
E17 阶段按 immutable full SHA + `main` ancestry 串行解锁。E17-0 已通过独立 Review 并完成 merge / push：Story SHA `abce4b712139c373f534a6fabab423fe138fc29c` 已是 `main` ancestor，merge commit 为 `2eee72cc44c2c7733cb565ea665ebfae48610085`，且 E17-0 合并完成时的基线已确认 `main...origin/main = 0 0`。当本 closeout immutable SHA 尚未成为 `main` ancestor 时，closeout 仍处于 Review / merge 门禁中，允许动作仅限独立 Review / merge，不得启动 E17-1。当本 closeout 通过独立 Review、merge / push，其 immutable SHA 成为 `main` ancestor，`main...origin/main = 0 0` 且 project-status、roadmap、readiness 与 E17 计划一致时，门禁自动满足。门禁满足后，主管理可通过 Git 解析 closeout immutable SHA 并生成 E17-1 提示词；不需要也不得创建“closeout 的 closeout”。E17-1 为 planned / prerequisite-gated；E17-2 至 E17-4 均 locked，E17-1 未完成前不得进入 E17-2，E17-4 通过前不得开始 production implementation，自动重连不作为默认前置。
...
E11.1 与 E16 已合入 Story 继续保留历史 Git 事实；E16 umbrella 已由 correct-course 关闭并被 E17 supersede，E16 原始代码和文档为 sealed historical archive / reference only。E16-10b-2 failed tip `89d1e23f870185a2e279d35bb293883f64fe70ba` 不是 `main` ancestor，永久禁止合并；b3 / b4 旧路线终止，E16-11 / E16-12 不自动进入 E17。浮动胶囊视觉与互动为 `adopted / frozen / direct reuse`，runtime 状态与接线不冻结。E17-0 已 reviewed / merged（Story SHA `abce4b712139c373f534a6fabab423fe138fc29c`；merge commit `2eee72cc44c2c7733cb565ea665ebfae48610085`）；E17-1 为 planned / prerequisite-gated，E17-2 至 E17-4 locked。closeout immutable SHA 尚未成为 `main` ancestor 时只允许独立 Review / merge；门禁满足后主管理可直接进入 E17-1 提示词生成。
...
```text
E15 系列已收口并保留原历史事实。E16 系列现为 `closed by correct-course / superseded by E17`，原始代码和文档为 sealed historical archive / reference only；已合入 Story 的 merge fact 保留，但不自动成为 E17 acceptance。E16-10b-2 `89d1e23f870185a2e279d35bb293883f64fe70ba` 永久禁止合并。胶囊视觉与互动为 `adopted / frozen / direct reuse`。E17-0 已 reviewed / merged（Story SHA `abce4b712139c373f534a6fabab423fe138fc29c`；merge commit `2eee72cc44c2c7733cb565ea665ebfae48610085`）。closeout immutable SHA 尚未成为 `main` ancestor 时，允许动作仅限独立 Review / merge；独立 Review、merge / push、immutable SHA ancestry、`main...origin/main = 0 0` 与权威文档一致性全部满足后，门禁自动满足，主管理可生成 E17-1 提示词。不需要也不得创建“closeout 的 closeout”。E17-1 为 planned / prerequisite-gated；E17-2 至 E17-4 仍 locked，E17-4 通过前禁止 production implementation。
```

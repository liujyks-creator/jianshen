...
31. E16-9：HeartRateState -> floating capsule live state mapping；official app shell 只读收集 provider/source/live state 并映射到浮动胶囊，状态优先级覆盖 hidden、权限未赋予、蓝牙关闭、未连接源、已选择设备、正在连接、等待数据、live bpm、数据过期、离线和 recoverable error。选择设备后仅在心率显示开启、BLE 权限已授予且 provider 内已有本次用户主动选择设备时调用 `connectSelectedDevice()`；胶囊点击 / 展开 / 拖动不触发 scan。固定 AVD `TrainFlow_Pixel_API_36` 已完成 disabled hidden、enabled no source、permission missing、fake saved source、expanded future-only recording copy 和 no scan / no crash smoke，证据目录为 `.local/smoke/e16-9-heart-rate-capsule-state-mapping/`。本 story 不做 cold-start saved-device 自动重连、不写 session record、不做 1s sampling persistence、不新增 Room / migration、不改 records/history/trends、commands/events、engines、TimerDial、声音、震动、通知或 cue。（Implemented; unit verified; AVD smoke completed）

...
- v2 start 只有在 bridge tests、legacy regression、unsupported-version fail-closed、rest extension、terminal record reconstructability 和 route gate 覆盖通过后才开放。该条件已由 E14.4-2b-5b-1 红测覆盖，E14.4-2b-5b-2 已实现 minimum engine bridge；下一步是 E14.4-2b-5b-3 v2 start gate enablement + smoke planning / implementation gate，不直接进入 TimerDial mapping。
...
- Legacy timed path 不变，unsupported / empty v2 fail closed，route v2 start gate 仍禁用。
- 本轮未改 route gate、TimerDial、Room schema、session record、`WorkoutCommand` 或 `WorkoutEvent`。下一步是 E14.4-2b-5b-3 v2 start gate enablement，不能直接进入 TimerDial mapping。
...
10. E14.4-2b-5b-3 v2 start gate enablement + smoke planning / implementation gate：下一步。只能在 bridge tests 和 regression 通过后开放 v2 start；仍不进入 TimerDial mapping。

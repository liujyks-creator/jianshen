...
2. **E17-1 Band 9 与标准 HRS 重新复验**：重新验证广播、UUID、CCCD、notify、payload、identifier 和 Huawei Health 互斥；真机事实不继承旧 E16 acceptance。本 closeout Story 开发期间不得启动，且它是设备/协议复验而不是 production implementation。（Planned / prerequisite-gated）
...
E17 阶段按 immutable full SHA + `main` ancestry 串行解锁。E17-0 已通过独立 Review 并完成 merge / push：Story SHA `abce4b712139c373f534a6fabab423fe138fc29c` 已是 `main` ancestor，merge commit 为 `2eee72cc44c2c7733cb565ea665ebfae48610085`，且该基线的 `main...origin/main = 0 0`。当前下一步只能是 E17-0 closeout docs-sync 独立 Code Review；该 closeout Review 通过、分支 merge / push、closeout immutable SHA 成为 `main` ancestor、`main...origin/main = 0 0` 且 project-status、roadmap、readiness 与 E17 计划一致后，主管理才可通过 Git 解析 closeout immutable SHA 并生成 E17-1 提示词。该交接不要求再创建“closeout 的 closeout”。E17-1 当前为 planned / prerequisite-gated 且未启动；E17-2 至 E17-4 均 locked，E17-1 未完成前不得进入 E17-2，E17-4 通过前不得开始 production implementation，自动重连不作为默认前置。

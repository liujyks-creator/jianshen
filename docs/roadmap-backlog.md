...
33. E16-10b：Heart-rate freshness / foreground reconnect implementation。已拆分为独立 implementation stories；E16-10b-1 policy/core 已实现并等待 review，foreground reconnect controller、UI mapper/settings copy（含 `停止连接`）以及 AVD / 真实 Band 9 验证均未开始。必须遵守 E16-10a 已批准参数，禁止自动扫描、后台连接、自动换 target，以及权限重新授予或 retry 耗尽后的自动恢复；不得进入记录或分析。（In progress）

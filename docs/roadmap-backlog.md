...
**范围:** Android MVP 与当前 React/Vite 原型承接。原始 Android MVP backlog 曾不包含真实可穿戴设备接入；D-080 已将用户显式 opt-in 后的标准 HRS 心率设备与冻结胶囊显示纳入当前 E17 产品范围，D-081 已确认唯一进程 owner + 活跃训练 `connectedDevice` foreground service 的最小架构。E17-4 已形成 `ready for implementation review` 候选结论，但当前仍为 `implemented / needs review`，production implementation locked；只有其 immutable SHA ancestry、独立 Review、merge / push、主分支同步与五份文档一致性全部完成且结论仍通过后，E17-5 gate 才自动 satisfied。该扩展不纳入全天候健康监测、Health Connect 历史同步、医疗告警、通用设备兼容承诺、自动恢复实现，或记录 / 分析 / 导出实现；后续商业化、云同步和完整课程平台也仍不在本范围。
...
5. **E17-4 Implementation readiness**：主文档为 `docs/planning/e17-4-heart-rate-implementation-readiness.md`。真实代码盘点已校正 627 行旧 provider、空 Application、Compose owner、debug旧provider consumer、三个 Route writer、Manifest / Service 缺口和胶囊适配范围；默认不新增 BLE operations seam，freshness 采用 E17-5 前台测量 -> E17-9 FGS 锁屏 / 后台补测 -> final lock 的无循环门禁。Review Repair 1 已固定E17-6/7 owner过渡、process visibility reducer、ID`7200`进程内generation ack、E17-10 evidence-only和E17-5 APK身份门禁。候选结论为 `ready for implementation review`。（Implemented / needs review；production locked）
6. **E17-5 Facts / freshness / presentation core**：唯一风险轴为三层事实与 public / capsule mapping、monotonic freshness 和旧 bpm 隔离；先完成前台 Band 9 测量 M0，只允许 provisional internal threshold，不发布最终 Live 合同。修改debug测量Activity后必须重新build/install并记录source preparation SHA、APK路径/variant/applicationId/Activity/SHA256、设备与分层日志；禁止沿用E17-1 APK，任何后续影响debug APK的可执行变化使旧M0证据失效。（Prerequisite-locked）
7. **E17-6 Deterministic Android BLE runtime owner**：唯一风险轴为 manual scan / connect、callback identity、generation / attempt、早到 callback、具体 Android BLE 调用与幂等 cleanup；新owner只在测试中实例化，旧provider/scanner/DTO继续维持现有production/debug接线编译并仍是唯一production可达BLE owner，不创建通用 GATT wrapper，不在本Story宣称旧runtime已retire。（Prerequisite-locked）
8. **E17-7 Application / settings / capsule production wiring**：唯一风险轴为 Application composition root、explicit opt-in / permission / picker、cold start、fail-closed process visibility与冻结胶囊外部适配；同一Story从`TrainFlowApplication`唯一创建点切换Application/Activity/Compose/settings/manual/saved-device/capsule和debug `HeartRateBroadcastSmokeActivity.kt`，退休旧scanner/GATT ownership及旧DTO production consumer，完成production基本Band 9 gate。visibility使用main-looper、Activity identity集合、configuration generation与确定性completion/timeout；E17-9前active training真实后台仍cleanup。（Prerequisite-locked）
9. **E17-8 Ordinary notification coordinator**：唯一风险轴为三个 Route writer 收口到 Application-scoped `ActiveWorkoutNotificationController` production instance；Route 只提交训练状态。（Prerequisite-locked）
10. **E17-9 Connected-device FGS / ID `7200` handoff**：唯一风险轴为 ordinary / FGS 单一 writer升降级、permission 分流、terminal ownership、Service / process death / Task Manager cleanup；release ack仅是Service在main-looper调用`stopForeground(STOP_FOREGROUND_REMOVE)`正常返回后的进程内generation事实，不是系统UI删除确认。ordinary writer等待当前匹配ack并只replay latest state；stale/duplicate/wrong-generation ack、失败或destroyed-before-ack不得创建第二writer。完成锁屏 / 临时后台 Band 9 gate并在 merge 前锁定 final freshness。（Prerequisite-locked）
11. **E17-10 Integrated AVD / Band 9 production acceptance**：evidence-only，不是implementation Story；production files/lines/methods均为0。只允许testing/evidence文档、`.local`设备证据，以及不改变production行为的test fixture/debug harness或断言修正。任何production finding返回E17-6/7/8/9独立Repair，合入同步main后重建APK并重跑全部受影响gate，旧APK/截图/日志不得复用。（Prerequisite-locked）

E17 阶段按 immutable full SHA + `main` ancestry 串行解锁。E17-0、E17-1、E17-2、E17-3 已 reviewed / merged。若 E17-4 immutable SHA 尚非 `main` ancestor，或独立 Implementation Readiness Review、merge / push、`main...origin/main = 0 0`、五份文档一致性任一未完成，则 E17-4 为 `implemented / needs review`，production implementation locked，只允许 E17-4 Review / Repair。全部门禁满足且候选结论仍通过后，E17-4 自动视为 `reviewed / merged`、E17-5 gate 自动 satisfied；之后 E17-5 至 E17-10 逐项要求前一 Story reviewed / merged、immutable SHA 成为同步 `main` ancestor。主管理从 Git 解析最终 SHA，不要求状态 docs-sync，不创建递归 closeout，不硬编码未来 merge commit。前台及活跃训练后台自动恢复为 `accepted product value / deferred implementation / requires separate product decision refinement and Story`，不是本轮 production 序列。
...
- E10.1/E10.2/E10.3 当时未包含真实心率设备接入；D-080 已将显式 opt-in 后的标准 HRS 与冻结胶囊显示纳入当前 E17 产品范围，但 E17-4 独立 Review / merge 门禁完成前 production implementation 仍 locked，其他设备路线及记录、分析、导出和自动恢复仍需独立后续 Story。

...
33. E16-10b：Heart-rate freshness / foreground reconnect implementation。已拆分为独立 implementation stories；umbrella 当前为 in progress。E16-10b-1 policy/core 已 reviewed / merged；E16-10b-2 已解锁但尚未开始，后续 UI mapping 与验证阶段继续锁定。必须遵守 E16-10a 已批准参数，禁止自动扫描、后台连接、自动换 target，以及权限重新授予或 retry 耗尽后的自动恢复；不得进入记录或分析。（In progress）
   ...
   - E16-10b-2：Foreground reconnect controller。已因 E16-10b-1 reviewed / merged 而解锁，允许创建独立 Dev Story；production provider 尚未消费 freshness policy，真实 timer、scheduler、watchdog、retry controller、callback race / old-target guard 与 lifecycle cancellation 均未实现，不得写成进行中或已验证。（Unlocked / Not started）
   - E16-10b-3：UI mapper / settings copy。继续依赖 E16-10b-2 的 runtime state / reason / attempt 边界；当前未实现 `停止连接`、reason / attempt 文案映射或 UI 状态接入。（Locked / Not started）
   - E16-10b-4：Verification / closeout。继续依赖 E16-10b-2 / E16-10b-3；当前没有 AVD non-BLE mapping closeout 或真实 Band 9 reconnect 验收，纯 policy JVM tests 不代表真实 BLE / reconnect readiness。（Locked / Not started）
...
E11.1 Heart-rate source boundary / unavailable state refinement 已实现并保留为底层边界；E16 广播开启 retest、E16-1 debug adapter spike、E16-2 production-capable provider hardening / Band 9 smoke、E16-3a 浮动胶囊视觉修订、E16-4 至 E16-9 opt-in / permission / device picker / floating capsule / live state mapping 均已按各自 gate 推进并保持记录与分析隔离。E16-10a docs-only policy 已 approved、reviewed / merged（merge commit `56d8029719889d329680f3dc099a77ae94909142`）；E16-10b umbrella 为 in progress。E16-10b-1 纯 Kotlin policy core 已 reviewed / merged（Story tip `09d17616f213c1df7905e46662f4a195345fdd9a`，merge commit `5cdee7ce1bd7a2b0f76f83adf069179a547fd16c`），但 production provider/runtime 尚未消费 policy。E16-10b-2 foreground reconnect controller 为 unlocked / not started；真实 timer、scheduler、watchdog、retry controller、callback race、old-target guard、lifecycle cancellation 和 Band 9 reconnect 验收均不存在。E16-10b-3 UI mapper/settings copy 与 E16-10b-4 verification / closeout 保持 locked / not started。E16-11 recording model / 1 秒持久化和 E16-12 analysis / recap 保持 not started 且继续隔离。E11.3 的首版心率显示、录入和统计撤销结论不变。
...
```text
E15 系列已收口：E15-5d 已 review / accepted / merged，merge commit `0fa28463e4c24bf039944402a209f8f55c922c1b`，story commit `d9875bd48cd3e51b560c677efc3f6d4440efc89a`，用户 APK 测试通过。后续开发者处理 E15 附近维护问题时，先读 `docs/testing/e15-maintenance-lessons-learned.md`。E16-9 已 reviewed / merged 到 main（`3271697fbc5c3d3385fbcdbc214f4d1a9a2c6832`），Band 9 修复后人工验收通过。E16-10a docs-only policy 已 approved、reviewed / merged（merge commit `56d8029719889d329680f3dc099a77ae94909142`）；E16-10b umbrella 现为 in progress。E16-10b-1 纯 Kotlin policy/core 已 reviewed / merged（Story tip `09d17616f213c1df7905e46662f4a195345fdd9a`，merge commit `5cdee7ce1bd7a2b0f76f83adf069179a547fd16c`），production provider/runtime 尚未消费该 policy。E16-10b-2 foreground reconnect controller 为 unlocked / not started；真实 timer、scheduler、watchdog、retry controller、callback race、old-target guard、lifecycle cancellation 和 reconnect 仍未实现。E16-10b-3 UI mapper/settings copy 与 E16-10b-4 verification / closeout 保持 locked / not started。E16-11 recording / 1s sampling persistence 与 E16-12 analysis / zones 保持 not started；不要直接进入 runtime reconnect、记录落库或分析。下一步只能执行本次 E16-10b-1 closeout docs-sync 分支的独立 Code Review gate；只有 docs-sync 合入 main 后，才能生成 E16-10b-2 Dev Story 提示词。
```

...
32. E16-10a：Heart-rate freshness / offline / reconnect policy planning。主管理已批准方案 B、10 秒 live stale、15 秒 waiting stale、30 秒 notify-abnormal、2/5/10 秒退避、最多 3 次、每次 10 秒 watchdog。有限 direct GATT reconnect 只对当前前台进程、本次已 live bpm 的同一 runtime target 生效，不 scan、不自动换 target；冷启动、回到前台、蓝牙恢复、权限重新授予和 retry 耗尽后均不自动创建 retry queue，不自动 scan/connect，也不恢复旧 attempt。权限丢失时取消 retry、stop scan、关闭 GATT并停止相关动作；重新授予后等待用户明确点击 `连接已保存设备` 或选择新设备。设置页需提供 `停止连接` 取消队列并 suppress 本前台周期自动恢复；retry exhausted 本身不产生事实状态，最近明确断开保持 `离线`，最近 connect / service discovery / CCCD / notify silence / parse 技术失败保持 `连接异常`，手动恢复文案不得改变底层状态。该 docs-only story 不实现 timer/retry/backoff、UI 操作或 BLE 行为，不新增 schema 或记录。（Implemented / Needs review; policy approved）
33. E16-10b：Heart-rate freshness / foreground reconnect implementation。已拆分为独立 implementation stories；umbrella 当前为 in progress。E16-10b-1 policy/core 已实现并完成 review-fix，当前等待最终 Review；后续 runtime controller、UI mapping 与验证阶段尚未开始。必须遵守 E16-10a 已批准参数，禁止自动扫描、后台连接、自动换 target，以及权限重新授予或 retry 耗尽后的自动恢复；不得进入记录或分析。（In progress）
   - E16-10b-1：Heart-rate freshness policy core。新增纯 Kotlin monotonic timeline / policy / stable reason codes，覆盖 waiting `<15s` / stale `15s..<30s` / first-sample silence `>=30s`，live `<10s` / stale `10s..<30s` / notify silence `>=30s`；valid bpm 重置 freshness，parse failure 不刷新，明确 disconnect 与技术失败事实分离，retry exhausted 不改事实，异常时间 fail closed。未接入生产 provider，不实现 timer、scheduler、callback race 或 reconnect；下一步只能进入 E16-10b-1 Code Review gate。（Implemented / Needs review）
   - E16-10b-2：Foreground reconnect controller。仅在 E16-10b-1 最终 Review 合并后解锁；production provider 尚未消费 freshness policy，真实 timer、scheduler、watchdog、retry controller、callback race / old-target guard 与 lifecycle cancellation 均未实现，不得提前启动。（Locked / Not started）
   - E16-10b-3：UI mapper / settings copy。继续依赖 E16-10b-2 的 runtime state / reason / attempt 边界；当前未实现 `停止连接`、reason / attempt 文案映射或 UI 状态接入。（Not started）
   - E16-10b-4：Verification / closeout。继续依赖 E16-10b-2 / E16-10b-3；当前没有 AVD non-BLE mapping closeout 或真实 Band 9 reconnect 验收，纯 policy JVM tests 不代表真实 BLE / reconnect readiness。（Not started）
34. E16-11：Heart-rate recording model / 1 秒持久化。尚未开始；E16-10a 不新增 `WorkoutSession`、Room、DataStore schema、records/history/trends 或心率样本。（Not started）
35. E16-12：Heart-rate analysis / recap。尚未开始；必须等待 E16-11 来源明确的已保存样本和独立 E16-12a 视觉 gate，E16-10a 不实现平均值、区间时长、曲线、复盘或训练建议。（Not started）
...
E11.1 Heart-rate source boundary / unavailable state refinement 已实现并保留为底层边界；Android `HeartRateState` / `HeartRateProvider` 可表达来源状态，但当前生产 UI、历史和统计不消费它。E11.2a 已完成 debug-only smoke：原条件下 Band 9 + 非华为 Android + 华为运动健康连接时仍无法发现华为设备，没有 BLE HRS / `0x2A37 notify` 物理证据。E16 广播开启 retest 在 18:46 用户截图中已形成正向证据链：`HUAWEI Band HR-OD7` 广播 `0x180D`、连接同一地址、发现 `0x180D` / `0x2A37 notify`、CCCD 写入成功并收到 bpm notify；E16 已 reviewed / merged 到 main，merge commit `bbd4296`。E16-1 已完成 debug adapter spike，E16-2 已完成 production-capable provider / state / permission / lifecycle / selected-device preference 地基并通过 2026-07-07 真机 smoke：debug entry 修复、HR Broadcast Smoke 可见但不污染首页、Band 9 scan / selected / connecting / waiting / notify / live bpm `84-91` / stop 均有截图结论。E16-3 初版 HTML 高保真视觉规划推荐顶部 pill，但该方向已被后续浮动胶囊决策取代；E16-3a App 内可拖动浮动心率胶囊 HTML 视觉修订已完成，merge commit `24c84c7`。E16-4 已完成 opt-in / settings / permission rationale / privacy / non-medical docs-only planning。E16-5 已完成 settings / opt-in UI，E16-6 已完成 BLE permission request flow：设置页只有在已开启心率显示并主动点击权限入口、看过 rationale 后才触发 runtime permission request，结果只回填 UI。E16-7 已完成 device picker / source status：设置页只在用户主动点击后执行 12 秒有限时 HRS scan，展示候选设备并仅保存 identifier / display name；当时仍不 connect、不订阅 notify、不读取 bpm、不接训练页、不记录。E16-8 已完成 app-shell floating capsule implementation：胶囊在 TrainFlow App 内 overlay 显示，支持 collapsed / expanded、tap、drag threshold、左右安全边 snap 和 fixed exclusion zones；mapper-ready 状态覆盖 bpm-only、zone+bpm、stale/offline 和 over-limit visual-only。固定 AVD `TrainFlow_Pixel_API_36` 已启动为 `emulator-5554` 并完成 AVD UI smoke；`bounds-check.txt` / `bounds-evidence.json` 为 `overall=PASS`，no-overlap / no-crash evidence 已有；rectangular shadow 已修复，shadow-fix smoke 已完成。E16-8 review fixes 已修复真机反馈：expanded 紧凑完整、debug APK 默认直进 TrainFlow、设置页授权 / 扫描 / 选择设备入口改为明确按钮，expanded 改为 `来源` / `记录` / `区间` / `更新` 信息面板，不再重复放置 `心率与设备` 主按钮；`记录` 格不再显示 `1s sampling`；胶囊拖动不再由 clickable 抢事件；runtime permission result 现在区分普通拒绝和永久拒绝，普通拒绝保留重新授权按钮，永久拒绝走系统设置。E16-9 已完成 provider/source/live state 到胶囊的只读映射，并且只在用户已开启心率显示、已授权蓝牙、已通过设置页主动选择本次 provider 已知设备后才允许 `connectSelectedDevice()`；胶囊点击、展开和拖动不触发 scan；仍不做 cold-start saved-device 自动重连、不写记录、不做分析。年龄、估算最大心率、可选手动最大心率、上限提醒阈值、区间说明和非医疗提示已作为后续心率个人参数设置记录，尚未实现。E16-10a freshness / offline / reconnect policy 已获批准并等待 review；E16-10b umbrella 已进入进行中，E16-10b-1 policy/core 已实现并完成 review-fix，当前为 implemented / needs review；E16-10b-2 foreground reconnect controller 仍 locked / not started，E16-10b-3 UI mapper/settings copy 与 E16-10b-4 verification / closeout 均 not started 并继续依赖前置阶段。production provider 尚未消费 policy，真实 timer、scheduler、watchdog、retry controller 或 Band 9 reconnect 验收均不存在。E16-11 recording model 和 E16-12 analysis 均未开始且保持拆分。E11.3 已按用户测试反馈撤销首版心率显示、录入和统计：执行页不再显示心率卡片，不提供手动输入，历史页不显示心率占位或旧平均心率趋势。
...
```text
E15 系列已收口：E15-5d 已 review / accepted / merged，merge commit `0fa28463e4c24bf039944402a209f8f55c922c1b`，story commit `d9875bd48cd3e51b560c677efc3f6d4440efc89a`，用户 APK 测试通过。后续开发者处理 E15 附近维护问题时，先读 `docs/testing/e15-maintenance-lessons-learned.md`，尤其是 1s / 2s short target route clock、力量休息 beep、auto-after-rest stage bell、selector 长中文、confirm-record 首屏、completed / abandoned sticky return、目标组颜色占位删除和力量执行短提示删除。E16 heart-rate broadcast retest 已合入 main（`bbd4296`），E16-1 debug adapter spike 和 E16-2 provider hardening 已完成且 E16-2 2026-07-07 真机 smoke 通过；E16-3a App 内可拖动浮动心率胶囊 HTML 视觉修订已合入（`24c84c7`），E16-4 opt-in / settings / permission rationale planning、E16-5 settings / opt-in UI、E16-6 permission request flow、E16-7 device picker / source status、E16-8 app-shell floating capsule 和 E16-9 provider/source/live state mapping 均已实现。E16-8 固定 AVD `TrainFlow_Pixel_API_36` smoke 已完成，设备为 `emulator-5554`，`bounds-check.txt` / `bounds-evidence.json` 为 `overall=PASS`，rectangular shadow fix 和 shadow-fix smoke 已完成；E16-8 review fix 已修复 expanded 过大、debug APK 默认 debug entry、最终信息面板、记录文案和永久拒绝权限路径，review-fix 证据路径为 `.local/smoke/e16-8-heart-rate-floating-capsule-review-fix/`。E16-9 已 reviewed / merged 到 main（`3271697fbc5c3d3385fbcdbc214f4d1a9a2c6832`），Band 9 修复后人工验收通过；只读接入 selected-device provider live state，不做冷启动自动重连、不自动扫描、不写记录。E16-10a policy 已获主管理批准，docs-only 状态为 implemented / needs review；权限丢失会取消 retry 并关闭 GATT，权限重新授予后不得自动 retry/scan/connect，必须由用户明确连接；retry exhausted 仅停止恢复并保持最近事实原因对应的 `离线` / `连接异常`。E16-10b umbrella 现为 in progress；E16-10b-1 policy/core 已实现并完成 review-fix，当前仍是 implemented / needs review，下一步只能重新执行 E16-10b-1 最终 Code Review。E16-10b-2 foreground reconnect controller 仍 locked / not started，E16-10b-3 UI mapper/settings copy 与 E16-10b-4 verification / closeout 均 not started 并继续依赖前置阶段。E16-11 recording / 1s sampling persistence 与 E16-12 analysis / zones 均未开始；不要直接进入 runtime reconnect、记录落库或分析。继续保持不恢复心率卡片、未获取心率、手动心率输入或旧平均心率趋势，不恢复力量目标组颜色占位。
```

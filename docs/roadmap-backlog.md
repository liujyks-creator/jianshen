...
1. 首页是否继续使用单 `app` module 内的轻量 Compose 状态与官方 shell，不提前引入完整导航架构。
2. E2.1 是否只做入口和首页信息架构，不提前实现计划编辑、训练执行或历史记录闭环。
3. 动作库入口是否复用 E1.3/E1.4 当前页面，并保持 fixture-only 数据源直到计划编辑或 repository story 明确推进。
4. 首页是否仍按 PRD/UX 保持计时训练默认推荐，同时让力量训练同层可见。

...
- E15-1a 已补齐 `auto_after_rest` 休息结束自动进入下一组 active set 时的阶段 bell：strength route 只在 tick 从 rest 自动进入 active 时为 `WorkoutEvent.StrengthSetStarted` 供应上一段 rest cue，dispatcher 复用既有 `STAGE_BELL` / `stage_bell_copper_clean.mp3` 生成请求；manual_start 休息结束、用户手动开始本组和休息中提前开始本组不被扩展成新铃声；`soundEnabled=false` 仍阻止请求。训练偏好仍只作为新建 / 编辑计划默认值，执行旧计划继续消费保存到计划中的 `StrengthExerciseBlock.setTimerMode`。
...
33. E13：E13.1 声音提醒与音频共存已实现；E13 sound cue asset / audio coexistence audit and QA gate 已完成；E15-1 已修复力量休息最后 N 秒 beep 与 `auto_after_rest` 休息后自动开始回归；E15-1a 已补齐 `auto_after_rest` 休息结束自动进入下一组 active set 的 `STAGE_BELL` 请求，蓝牙 / 扬声器真机 smoke 仍待继续补证据，固定女声 cue、力量完成 bell 和力量 haptics parity 留给后续。

...
33. E16-10b：Heart-rate freshness / foreground reconnect implementation。尚未开始；负责可测试 policy/core、foreground reconnect controller、UI mapper/settings copy（含 `停止连接`）以及 focused tests / Android checks / AVD 与真实 Band 9 验证收口。必须遵守 E16-10a 已批准参数，禁止自动扫描、后台连接、自动换 target，以及权限重新授予或 retry 耗尽后的自动恢复；不得进入记录或分析。（Not started）

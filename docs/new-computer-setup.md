...
它们是旧电脑上的本地路径，不会因为克隆本仓库自动出现在新电脑。

新电脑继续使用它们时，任选一种方式：

1. 把技能复制到新电脑同类路径，并在 Codex 中引用新路径。
2. 把这些技能放进你自己的私有 skills 仓库，再在新电脑安装或同步。
3. 如果某个技能只服务本项目，并且允许入库，再把必要说明整理进本仓库。

对新 Codex 会话可以这样说明：

```text
产品设计与规划阶段，如本机已存在 bmad-method 和 design-md 技能，请先读取它们再继续。

...
- Tracked planning sync 已在 accepted base `abf85553bc0c4a71793858734af265b634caab69` 满足，主管理已选择 root Story `E17-CS-03`。当前 branch=`codex/e17-cs-03-canonical-schema-migration-foundation`，结论=`REPAIR_IMPLEMENTED_CANDIDATE / NEEDS_FRESH_INDEPENDENT_REVIEW / NOT_MERGED`。
...
- 只有 fresh independent Review PASS、`--no-ff` merge / push、candidate full SHA 成为同步 `main` / `origin/main` ancestor、`main...origin/main=0 0` 且状态文档一致后，CS-03 才可自动提升为 `reviewed / merged`。Candidate 期间 CS-04 仍为 `planned / prerequisite-gated`；不得提前准备或启动 CS-04。

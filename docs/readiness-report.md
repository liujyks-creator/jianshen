...
> 2026-07-10 刷新：E16-9 HeartRateState -> floating capsule live state mapping 已实现，主文档为 `docs/testing/e16-9-heart-rate-capsule-state-mapping.md`，AVD smoke 证据目录为 `.local/smoke/e16-9-heart-rate-capsule-state-mapping/`。当前 readiness 结论仍不是训练记录落库或分析许可：本轮只把 provider/source/live state 只读映射到 App 内浮动胶囊，并在用户已开启心率显示、已授权蓝牙、已通过设置页主动选择本次 provider 已知设备后，允许 provider lifecycle 进入 `connectSelectedDevice()`；胶囊点击、展开和拖动不触发 scan。胶囊可显示 `正在连接`、`等待数据`、live bpm、`数据过期`、`离线` 和 recoverable error，但不写 `WorkoutSession`、不做 1s sampling persistence、不新增 Room / migration、不改 records/history/trends、`WorkoutCommand` / `WorkoutEvent`、TimedWorkoutEngine、StrengthWorkoutEngine、TimerDial、声音 / 震动 / 通知或 cue。冷启动仅凭 saved identifier 自动重连仍未实现，需后续另拆；E16-10 stale / offline policy、E16-11 recording model 和 E16-12 analysis 仍需单独 story。

...
> 2026-07-12 E16-10b-1 closeout 记录：policy core 已 reviewed / merged（Story tip `09d17616f213c1df7905e46662f4a195345fdd9a`，merge commit `5cdee7ce1bd7a2b0f76f83adf069179a547fd16c`）。该节点只合入 pure policy；其后 production runtime接入状态以上方最新E16-10b-2刷新为准。

> 2026-07-12 E16-10b-2 刷新：foreground reconnect controller 已 implemented / needs review。production provider现已消费 monotonic freshness policy，并通过单一 pure controller + main Handler adapter实现2 / 5 / 10 秒最多三次 direct retry、10 秒 connect-stage watchdog、old-GATT/target/attempt guard、intentional close和完整 cancellation；deterministic HeartRate tests与固定 AVD non-BLE smoke通过。AVD 无真实 BLE HRS target，不能替代 Band 9 reconnect / exhaustion / manual recovery closeout。E16-10b umbrella仍 in progress；b3/b4 locked / not started，E16-11/E16-12 not started。下一步只能进入 b2 Code Review gate。
...
- E16-10a policy 已 approved、reviewed / merged，参数保持10 / 15 / 30秒freshness、2/5/10秒最多三次retry与10秒watchdog。E16-10b-1 policy core已 reviewed / merged；E16-10b-2 production foreground controller已 implemented / needs review，自动测试与AVD non-BLE smoke通过但真实Band 9 closeout未完成。E16-10b-3 mapper/settings copy与E16-10b-4 real-device verification locked / not started；E16-11 / E16-12 not started。

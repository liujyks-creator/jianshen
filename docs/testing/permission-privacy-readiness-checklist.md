...
**状态:** E9.2 权限与隐私文案收口
**适用范围:** 设置页、计划提醒、活跃训练通知、健康数据边界、恢复建议、音频提示、语音预留、内存态数据边界。
...
- 当前首版不显示心率、不录入心率、不统计心率，也未接入真实设备、手环、手表或健康数据。
- 未来健康数据不得做医疗告警、危险判断或训练强度判断。
...
- 计时、力量和基础跟练执行页不显示心率说明、心率占位或手动心率输入；未来健康数据不得抢当前动作、时间、组目标和主按钮。

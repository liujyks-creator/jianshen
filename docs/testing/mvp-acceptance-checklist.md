...
- **Partial:** E10.10 后计时/力量计划保存、计划列表、复制、删除和提醒更新已接入本地 Room repository；编辑回填、完整跟练编排、历史清理、统计图表、长期恢复回顾和部分分析能力仍是后续 story。
- **Pass:** E9.4 已修复计划编辑页数字输入临时清空和编辑页立即开始：计时/力量编辑页的数字草稿可为空，空值时保存/开始禁用并显示原因；有效草稿可直接进入对应执行页。
- **Pass:** E10.2 后计时训练已重构为纯间歇阶段编辑；E10.10 后自定义阶段、轮次、颜色、图标和提醒设置可本地保存与恢复。
- **Product Decision:** 基础跟练后续应承担动作选择、动作推荐、排序和内容展示；动作选择后续应进入独立动作选择页面。这些归入 E10，不在 E9.3 修改。
- **Deferred:** 总统计图表、计划级多次趋势、历史记录全部清除 / 按计划清除 / 按日期清除、后台可靠计时、进程死亡恢复、真实心率设备、Health Connect / Wear OS / BLE、语音控制和完整课程平台均不进入 E10.10 当前实现。
...
| 项目 | 状态 | 结论 |
|---|---|---|
| 创建计时训练计划 | Pass | E10.10 已接入本地 `WorkoutPlan` repository 保存和恢复；编辑回填仍归后续 story。 |
...
| 项目 | 状态 | 结论 |
|---|---|---|
| 创建力量计划 | Pass | E10.10 已接入本地 `WorkoutPlan` repository 保存和恢复；编辑回填仍归后续 story。 |
...
| 用户反馈 | Product Decision | 后续 E12 需要总统计、图表和计划调整证据；首版不再规划平均心率趋势。 |

### F. 健康数据 / 心率边界

| 项目 | 状态 | 结论 |
|---|---|---|
| 首版 UI | Pass | E11.3 后执行页不显示心率卡片，不提供手动心率输入。 |
| 历史 / 统计 | Pass | 记录页不显示心率占位，不绘制平均心率趋势。 |
| 无医疗/危险/强度判断 | Pass | 当前不基于心率或健康数据驱动训练中断、医疗化告警或强度建议。 |
| 后续方向 | Deferred | 若未来重启设备方向，需独立评估手环、手表、Health Connect、BLE、SDK 权限和 UI 空间。 |
...
- 测试者需要知道：计时/力量计划已本地持久化；历史清理、长期恢复回顾、完整跟练编排和部分分析能力仍多为内存态、fixture 或基础展示。
- 测试者需要知道：当前首版不显示心率、不录入心率、不统计心率；恢复建议也不是医疗或康复建议。
...
4. E11：健康数据边界已收口；首版不显示、不录入、不统计心率，未来设备策略需独立重启。
5. E12：数据分析趋势，补总统计、图表、计划级趋势、计划调整证据和记录清理能力。

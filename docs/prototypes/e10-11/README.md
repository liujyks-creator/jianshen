...
黑场、亮红玻璃质感运动强调、白色总进度线。适合验证用户偏好的强训练现场感，也最接近“减少文字、靠颜色和圆盘读状态”的方向。建议把高对比层级、中心阶段色填充、final 5 seconds 的轻量脉冲作为候选，但不要把过强黑红情绪直接定为 MVP 默认。
...
- 外圈、内圈和中心圆应形成更紧凑的同心仪表盘比例；中心圆盘和内部元素可以更大，贴近浅色底环但不能触碰进度圆圈 marker。
- 进度圆圈、浅色底层圆环和阶段 marker 这一组应靠近最外侧阶段圆圈，只保留窄缝；当前 HTML 原型使用约 `315` 的进度组半径。
...
- Work 阶段红色可探索更亮的玻璃感处理，包括径向亮面、顶部高光和轻微边缘透亮，但生产实现应保持可读性优先。

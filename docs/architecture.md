...
- E16-10a 已完成 freshness / offline / reconnect policy 规划，等待主管理对话确认，详见 `docs/testing/e16-10-heart-rate-freshness-reconnect-policy.md`。推荐方向只允许当前前台进程中已进入 live 的同一 runtime target 进行有限 direct GATT reconnect，且不 scan、不按保存 identifier 自动发现、不切换 target；冷启动、回到前台、蓝牙恢复和 retry 耗尽后均不自动 scan/connect。10 / 15 / 30 秒 freshness 与 2 / 5 / 10 秒 retry 参数仍是待批准候选值，E16-10b 不得先行实现。

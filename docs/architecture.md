...
- E16-10a freshness / offline / reconnect policy 已获批准，docs-only 状态为 implemented / needs review，详见 `docs/testing/e16-10-heart-rate-freshness-reconnect-policy.md` 和 D-078。只允许当前前台进程中本次已进入 live bpm 的同一 runtime target 进行有限 direct GATT reconnect，且不 scan、不按保存 identifier 自动发现、不切换 target；冷启动、回到前台、蓝牙恢复和 retry 耗尽后均不自动 scan/connect。默认 freshness 为 10 秒 live stale、15 秒 waiting-first-bpm stale、30 秒 notify silence -> `连接异常`；明确 GATT 断开 -> `离线`。重连预算为 2 / 5 / 10 秒退避、最多 3 次、每次 10 秒 watchdog。设置页必须提供 `停止连接` 以取消队列并 suppress 本前台周期自动恢复；retry exhausted 沿用既有 `离线` / `连接异常` 历史事实状态。E16-10b 尚未开始，负责可测试 policy/core、foreground reconnect controller、UI mapper/settings copy 和验证收口；E16-10a 不代表这些能力已经实现。

...
- E16-10a freshness / offline / reconnect policy 已 approved、reviewed / merged（merge commit `56d8029719889d329680f3dc099a77ae94909142`），详见 `docs/testing/e16-10-heart-rate-freshness-reconnect-policy.md` 和 D-078。只允许当前前台进程中本次已进入 live bpm 的同一 runtime target 进行有限 direct GATT reconnect，且不 scan、不按保存 identifier 自动发现、不切换 target；冷启动、回到前台、蓝牙恢复、权限重新授予和 retry 耗尽后均不自动创建 retry queue，不自动 scan/connect，也不恢复旧 attempt。默认 freshness 为10 / 15 / 30秒，重连预算为2 / 5 / 10秒最多三次，connect-stage watchdog为10秒；retry exhausted不产生新事实状态。E16-10b-1 pure policy core已 reviewed / merged，production runtime由下方E16-10b-2接入。
- E16-10b-2 已 implemented / needs review：`HeartRateForegroundReconnectController` 是 current runtime target、freshness timeline、retry budget、watchdog与 cancellation 的唯一 owner；pure controller只依赖 monotonic clock与可取消 scheduler，Android provider以 `SystemClock.elapsedRealtime()`及main Handler作为 adapter。每个 direct `connectGatt` 使用独立 callback，并同时校验GATT对象身份、target generation和attempt generation；intentional close先失效 token再 disconnect/close。provider只对本进程中曾产生valid bpm的同一runtime `BluetoothDevice`做2 / 5 / 10 秒最多三次retry，controller无scan effect；foreground/display/permission/Bluetooth/stop/target/scan/close取消后不自动恢复。b3最终UI mapper/settings copy和b4真实Band 9 closeout仍未开始。

...
`Story E4.1: 力量训练执行引擎`、`Story E4.2: 力量训练执行页`、`Story E4.3: 单组完成确认层`、`Story E4.4: 动作替换与跳过`、E4 UI smoke fix、`Story E5.1: 计时训练总结`、`Story E5.2: 力量训练总结`、`Story E5.3: 训练历史与基础趋势`、`Story E5.4: 基础恢复建议`、`Story E6.1: 跟练雏形计划入口`、`Story E6.2: 跟练执行页`、`Story E6.3: 心率抽象状态展示`、`Story E7.1: 训练提醒通知`、`Story E7.2: 活跃训练通知边界`、`Story E7.3: 训练偏好设置`、`Story E8.1: Skin contract and registry`、`Story E8.2: Tile Flow 完整视觉重做` 和 `Story E8.3: Big Type 完整视觉重做` 均已通过 Review Gate 并合入 `main`。当前阶段分支 `codex/e8-4-ui-skin-review-checklist` 基于 `main` / `origin/main` 的 `60b25d6cdbc1bda7d9248b5ed421182207cc31af` 实施 E8.4：新增 UI skin review 与 E9 用户测试前 readiness checklist，补充社区 UI 定制审查边界，并用轻量测试保护三套内置皮肤 registry、metadata、默认 skin、未知 id 回退和 mode pill 对比度。当前无已知 blocker。
...
12. `docs/planning/action-content-slice.md`
...
1. 完成 `codex/e8-4-ui-skin-review-checklist` 分支 Review Gate。
2. 对照 `docs/testing/ui-skin-readiness-checklist.md` 准备 E9 用户测试前 UI readiness：三套内置皮肤、小屏主控制、权限说明、心率非医疗化、恢复建议非医疗化和普通通知边界。
3. 将用户测试回看事项纳入 E9：热身/动作/动作后休息/轮间休息/放松拉伸是否都应支持最后 N 秒提醒，以及训练提示音不降低、暂停或打断其他 App 音乐/视频。

...
1. 进入 `Story E6.3: 心率抽象状态展示` Review Gate。
2. Review Gate 重点检查计时、力量和跟练执行页是否复用同一 `HeartRateDisplayUiState` mapper。
3. 确认 disabled、not_connected、connecting、available、stale、error 六种状态文案克制，且 available 可显示 bpm。
4. 确认未新增健康、传感器或蓝牙权限，也未接入真实设备、心率告警、热量判断或训练强度判断。

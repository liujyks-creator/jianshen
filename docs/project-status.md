...
后续若继续 E14.4-2，应优先处理仍低耦合的计时阶段卡密度、添加阶段 / 添加动作选择体验，以及计划颜色 / 力量目标组颜色的独立持久化决策；不得在普通 UI polish 中静默改 Room schema。计时训练 UI / TimerDial 结构优化仍拆为独立 **E14.4-2b Timed composition editor and TimerDial ring semantics**：外层目标编排 + 内层阶段 segment 两层结构，外层目标可新增 / 删除 / 拖动 / 重命名 / 设色，内层阶段可新增 / 删除 / 拖动 / 重命名 / 设置时长和颜色，外层总时长由内层阶段求和，TimerDial 外圈按内部阶段时长占比分段表达。E14.4-2b 必须先规划 / 视觉确认，再代码实现；不得在 E14.4-2 中静默修改 Room schema、训练引擎、`WorkoutCommand`、`WorkoutEvent`、session record、声音语义或统计比较 key。

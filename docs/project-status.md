...
5. E12 Stats / Records 已具备真实基础统计、非心率聚合图表、历史清理和计时同类阶段 / 额外休息趋势；后续若继续 E12，应优先完成 E12.2b 力量同类 set 趋势 review/fix 或另行拆分同日多轮分析，仍不得回填或修改原计划结构。E12.2b planned lookup 必须限定同一 `exerciseId`：`sourceSetPlanId` 优先，只有缺失时才可按 `setOrder + setKind` fallback；存在但未匹配时是数据不足；替换动作 planned values 只能来自 `substitutedFromExerciseId` 对应原动作 block，并且必须标注替换来源。平均心率趋势必须等待明确来源的设备心率或可选手动心率数据；如果两者都没有，历史页和趋势页显示未获取心率，不画假趋势。

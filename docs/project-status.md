...
1. 进入 `Story E5.4: 基础恢复建议` Review Gate。
2. Review Gate 重点检查恢复建议是否只基于本次训练动作的 fixture recovery 映射和内存态 session/summary 数据。
3. 确认“查看恢复建议”入口已从计时/力量总结进入恢复页，并在无可识别动作时展示诚实空状态。
4. 确认文案保持训练后放松方向，不生成医疗诊断、康复治疗承诺、自动训练建议、心率判断、热量判断或疾病适应性判断。

...
1. 进入 `Story E4.3` Review Gate，重点检查确认层是否只通过 E4.1 `StrengthWorkoutEngine` 和 `WorkoutCommand.ConfirmStrengthSet` 生成实际组记录。
2. Review Gate 通过后再合入 `main`，再启动 `Story E4.4: 动作替换与跳过`。
3. E4.4 应保持替换/跳过与核心引擎命令边界一致，不把替换动作或跳过记录写成 UI 私有状态。

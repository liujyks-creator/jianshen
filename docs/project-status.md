...
1. 完成 `codex/e9-preference-explicit-settings-regression` 小回归保护分支 Review Gate。
2. 确认新增测试覆盖新建计时/力量计划会消费默认偏好，已有草稿/计划中的显式 `CueSettings` 和 set timer mode 不被静默覆盖。
3. 确认本阶段只新增窄范围回归测试；不启动 E8 UI 改版，不扩展通知、foreground service、exact alarm、语音、心率设备或 session records 持久化。

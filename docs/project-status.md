...
5. 后续心率数据源策略和健康数据权限流；E11.1 已完成 source-aware UI/provider boundary，仍不接真实设备、不做手动输入、不持久化心率。E11.2a 需要先做 HUAWEI Band 9 on non-Huawei Android feasibility smoke：当前 Band 9 + 非华为 Android + 华为运动健康可读数据只证明华为运动健康能读取设备，不证明 TrainFlow 能实时读取；先验证 BLE HRS `0x180D` / `0x2A37 notify`，若不可用再验证 Huawei Health Kit / Health Service Kit 在非华为 Android + Band 9 + HMS Core 条件下的实时授权读取。Apple Watch / HealthKit 保留为未来 iOS 第一优先路线；Health Connect 更适合历史摘要 / 趋势，不作为 E11.2a 实时执行页首选。
...
5. E11.1 Heart-rate source boundary / unavailable state refinement 已完成；下一步建议做 E11.2a HUAWEI Band 9 on non-Huawei Android feasibility smoke，再决定 BLE HRS adapter spike、Huawei SDK feasibility、历史同步或暂不接设备路线。E11.2a 前不得直接做生产设备接入；smoke 本身不持久化心率、不画平均心率趋势、不申请生产健康 / 蓝牙 / 身体传感器权限，也不让 BLE、Huawei、Health Connect、HealthKit、Wear OS 或厂商 SDK model 泄漏到 TrainFlow UI / history / analytics。E11.3 可选手动心率输入继续后置。
6. E12 Stats / Records 已具备真实基础统计、非心率聚合图表、历史清理、计时同类阶段 / 额外休息趋势和力量同类 set 趋势；后续若继续 E12，可另行拆分同日多轮分析，仍不得回填或修改原计划结构。平均心率趋势必须等待 E11 后续明确来源的设备心率或可选手动记录；如果两者都没有，历史页和趋势页显示未获取心率，不画假趋势。
7. E13.1 Sound Cue System 已实现短提示音播放与音频共存；后续可进入 E13.1 Review Gate，并补真实设备上的手机扬声器 / 蓝牙耳机 smoke 记录。E13.2 固定女声 cue / 阶段名朗读仍是后续增强，不做用户任意 TTS 或自动语音教练。
...
E6.3 新增心率抽象状态展示；E11.1 已把该 baseline 收口为 source-aware 状态。`feature.workoutsession` 现在通过共享 `HeartRateDisplayUiState` / mapper 消费 `HeartRateState`，计时训练、力量训练和基础跟练执行页统一展示无来源、设备等待读数、设备读数、手动读数、过期读数、provider 不可用和权限不可用。设备或手动当前读数可显示 bpm，measuredAt、recordedAt、sourceId、sourceLabel 和 message 只作为低层级辅助信息；`warningLevel` 告警口径已废弃，不通过心率状态驱动颜色、告警、训练状态或控制按钮。`core.health` 保留 `HeartRateProvider`、`DisabledHeartRateProvider` 和 `MockHeartRateProvider` 边界，仅输出抽象状态，不接 HealthKit、Huawei、Health Connect、Wear OS、BLE、厂商 SDK、真实传感器或健康/蓝牙权限。当前训练闭环在无来源、等待读数、数据过期或来源不可用状态下仍完整可用。
...
E11.1 Heart-rate source boundary / unavailable state refinement 已实现；Android `HeartRateState` 从旧 availability baseline 收口为 `kind + sourceKind` source-aware 状态，可表达无来源、设备已连接但等待读数、设备读数、手动读数、过期读数、provider 不可用和权限不可用。`DisabledHeartRateProvider` 默认输出 no source / unavailable；`MockHeartRateProvider` 可表达设备、手动、过期和不可用抽象状态，但不代表真实设备。计时、力量和基础跟练执行页共享心率 mapper：无来源显示 `-- bpm` / `未获取心率`，设备读数显示 `设备数据`，手动读数显示 `手动录入`，过期读数保留旧 bpm 并标注 `数据已过期`。后续 Apple Watch / iOS 走 iOS app + watchOS companion + HealthKit / HKWorkoutSession / HKLiveWorkoutBuilder；HUAWEI Band 9 当前只作为非华为 Android feasibility 样本，先验证 BLE HRS `0x180D` / `0x2A37 notify`，不可用时再验证 Huawei Health Kit / Health Service Kit 实时授权读取；Health Connect 更适合历史摘要 / 趋势。所有 adapter 都必须统一输出 TrainFlow `HeartRateState` 并标注来源。E11.1 仍未接真实设备、HealthKit、Huawei、BLE、Health Connect、Wear OS 或厂商 SDK，未实现手动输入 UI，未申请真实健康 / 蓝牙 / 身体传感器权限，未持久化心率，未绘制平均心率趋势，未改变 `WorkoutCommand` / `WorkoutEvent` / 训练引擎语义，也不做医疗判断、危险告警或训练中断依据。下一步推荐 E11.2a HUAWEI Band 9 on non-Huawei Android feasibility smoke；E11.3 可选手动输入保持后置。

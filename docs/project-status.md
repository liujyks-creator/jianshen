...
E13.1 Sound cue playback and audio coexistence 已实现；`core.media` 新增事件到声音提示请求的纯 Kotlin 映射、重复事件去重 controller 和音频共存策略断言，计时与力量执行页只消费既有 `WorkoutEvent` / `CueSettings`，不修改 `WorkoutCommand` / `WorkoutEvent` / engine 语义。最后 N 秒按 `CountdownCue.thresholdSec` 与 `soundEnabled` 映射：`countdown_beep1.mp3` 的 App raw 副本用于最后 N 秒前几声 beep，`stage_bell_copper_clean.wav` 的 App raw 副本用于最后 1 秒、阶段切换和休息开始铃声。两个素材来源已由用户确认为用户本人 / 项目内制作，授权用于 TrainFlow App 内短提示音分发；提交边界只包含 `app/src/main/res/raw/` 的 Android raw 资源副本，不提交根目录 `countdown_beep1.mp3` 原文件或 `.local/audio/stage_bell_copper_clean.wav` 原文件。Android 播放层使用 `SoundPool` 与 sonification audio attributes，不请求 audio focus，不主动 duck，不暂停外部音乐 / 视频，并在 route dispose 时释放播放器。本阶段未实现固定女声 cue、TTS、语音教练、notification action、foreground service、exact alarm、心率设备、云同步、账号体系或新训练引擎语义；真实扬声器 / 蓝牙耳机 smoke 仍需设备回归记录。
...
11. E13.1 已接入两个用户本人 / 项目内制作且授权用于 TrainFlow App 内短提示音分发的素材副本：`app/src/main/res/raw/countdown_beep1.mp3` 用于最后 N 秒前几声 beep，`app/src/main/res/raw/stage_bell_copper_clean.wav` 用于最后 1 秒、阶段切换和休息开始铃声；根目录 `countdown_beep1.mp3` 原文件与 `.local/audio/stage_bell_copper_clean.wav` 原文件仍不得提交。

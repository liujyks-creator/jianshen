...
`Story E4.1: 力量训练执行引擎`、`Story E4.2: 力量训练执行页`、`Story E4.3: 单组完成确认层`、`Story E4.4: 动作替换与跳过`、E4 UI smoke fix、`Story E5.1: 计时训练总结`、`Story E5.2: 力量训练总结`、`Story E5.3: 训练历史与基础趋势`、`Story E5.4: 基础恢复建议`、`Story E6.1: 跟练雏形计划入口`、`Story E6.2: 跟练执行页`、`Story E6.3: 心率抽象状态展示`、`Story E7.1: 训练提醒通知`、`Story E7.2: 活跃训练通知边界` 和 `Story E7.3: 训练偏好设置` 均已通过 Review Gate 并合入 `main`。当前阶段分支 `codex/e8-1-skin-contract-registry` 基于 `main` / `origin/main` 的 `7368cccac397b7bd214ff839abf2406df1d72cb4` 建立 E8.1 内置 UI 皮肤 contract、registry、DataStore preference、设置页选择入口和 theme token 映射。当前无已知 blocker。
...
1. 完成 `codex/e8-1-skin-contract-registry` 分支 Review Gate。
2. 确认 Official Flow 仍符合 `DESIGN.md`，Tile Flow / Big Type 仅作为内置注册占位和轻量 token 差异存在。
3. 确认本阶段只新增皮肤切换机制、metadata、DataStore preference、设置入口和测试；不启动 Tile Flow / Big Type 完整页面重做，不扩展运行时插件市场、远程皮肤、通知、foreground service、exact alarm、语音、心率设备或 session records 持久化。

...
1. 进入 `Story E5.1: 计时训练总结` Review Gate。
2. Review Gate 重点检查 summary 是否只消费 E3.4 history 边界，是否诚实说明 active elapsed 不是 wall-clock startedAt / endedAt。
3. 确认恢复建议入口仍是 E5.4 占位，没有暗示完整建议已生成。
4. 后续如 E5.1 通过，再进入 E5.2 力量训练总结；不要跳到 E5.3 历史趋势或 E5.4 完整恢复建议。

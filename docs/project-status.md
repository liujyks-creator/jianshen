...
1. 进入 `Story E4.2` Review Gate，重点检查力量执行页是否只通过 E4.1 `StrengthWorkoutEngine` 推进 prepare / active / confirm / rest / completed / abandoned。
2. Review Gate 通过后再合入 `main`，再启动 `Story E4.3: 单组完成确认层`。
3. E4.3 应在现有 `confirm_strength_set` 边界上增加可编辑重量、次数、感受和备注，不改写 E4.1 引擎语义。

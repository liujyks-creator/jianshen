...
1. 进入 `docs/roadmap-backlog.md` 中的 `Story E2.4: 计划列表、详情、复制与删除`，在 E2.2/E2.3 已完成的 `WorkoutPlan` 草稿契约基础上接入计划管理能力。
2. E2.4 可以接入计划列表、计划详情、复制和删除，但不提前实现训练执行引擎、`WorkoutSession` / session records、通知调度、真实心率设备接入、voice control 或跟练闭环。
3. E2.4 的 repository、Room/DataStore 持久化和 feature/UI 边界以 E2.4 story 与现有架构为准，不静默扩大为完整训练执行或记录闭环。

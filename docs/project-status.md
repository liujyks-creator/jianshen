...
`Story E4.1: 力量训练执行引擎`、`Story E4.2: 力量训练执行页`、`Story E4.3: 单组完成确认层`、`Story E4.4: 动作替换与跳过`、E4 UI smoke fix、`Story E5.1: 计时训练总结`、`Story E5.2: 力量训练总结`、`Story E5.3: 训练历史与基础趋势`、`Story E5.4: 基础恢复建议`、`Story E6.1: 跟练雏形计划入口`、`Story E6.2: 跟练执行页`、`Story E6.3: 心率抽象状态展示`、`Story E7.1: 训练提醒通知`、`Story E7.2: 活跃训练通知边界`、`Story E7.3: 训练偏好设置`、`Story E8.1: Skin contract and registry` 和 `Story E8.2: Tile Flow 完整视觉重做` 均已通过 Review Gate 并合入 `main`。当前阶段分支 `codex/e8-3-big-type-skin` 基于 `main` / `origin/main` 的 `9264466c7cc159c00d7308c7e8be06d8412e59d0` 实施 E8.3 Big Type：训练首页使用少信息大入口，两类训练执行页放大当前动作、主时间/本组目标和主按钮，力量页把暂停/继续与结束训练纳入 Big Type 固定底部控制区，确认层使用单列输入保护小屏可用性。当前无已知 blocker。
...
1. 完成 `codex/e8-3-big-type-skin` 分支 Review Gate。
2. 确认 Big Type 首页、设置页、计时执行页、力量执行页和力量确认层在小屏幕与常规手机尺寸下可读、可点按且主控制即时可见。
3. 确认 Official Flow 与 Tile Flow 无视觉和行为回归，并且本阶段未扩展运行时插件市场、远程皮肤、通知、foreground service、exact alarm、语音、心率设备或 session records 持久化。

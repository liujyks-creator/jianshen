...
5. 后续心率数据源策略和健康数据权限流；E11.1 已完成 source-aware UI/provider boundary，仍不接真实设备、不做手动输入、不持久化心率。E11.2 需要继续做 Apple Watch / iOS HealthKit + workout session adapter、Huawei Health feasibility、BLE Heart Rate Service adapter 以及 Health Connect / Wear OS / 厂商 SDK 的权限、来源、设备范围和失败状态决策。
...
5. E11.1 Heart-rate source boundary / unavailable state refinement 已完成；下一步建议进入 E11.1 Review Gate，再按优先级推进 E11.2 真实设备接入策略 / adapter feasibility 或 E11.3 可选手动心率输入。E11.2 前不得直接接 HealthKit、Huawei、BLE、Health Connect、Wear OS 或厂商 SDK，也不得申请真实健康 / 蓝牙 / 身体传感器权限。

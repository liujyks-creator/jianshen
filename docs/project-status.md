...
1. 进入 `Story E4.1` Review Gate，重点检查力量引擎状态推进、确认草案、默认回填、休息记录、暂停冻结和 terminal 边界。
2. Review Gate 通过后再合入 `main`，再启动 `Story E4.2: 力量训练执行页`。
3. E4.2 应复用 `StrengthWorkoutEngine` 和 `WorkoutCommand` / `WorkoutEvent`，不要在 UI 中手写第二套力量状态机。

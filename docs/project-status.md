...
1. 进入 `Story E7.1: 训练提醒通知` Review Gate。
2. Review Gate 重点检查计划详情能设置/关闭训练提醒时间，且 Android 13+ 通知权限关闭时有清楚提示。
3. 确认提醒通知 channel、通知内容和 UI 文案均说明首版是普通通知，允许系统延迟，不承诺闹钟级强提醒。
4. 确认未新增 exact alarm、foreground service、健康、传感器、蓝牙或定位权限，也未实现 E7.2 活跃训练 ongoing notification。

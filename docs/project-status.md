...
1. 完成 E2.4 review gate 后，将阶段分支按主管理对话流程验收并合入 main。
2. 下一轮建议进入 `Story E3.1: 计时训练执行引擎`，先实现可测试的计时状态推进、动作/休息临近结束事件、暂停/继续边界，不把 UI、通知或真实 session records 混入引擎核心。
3. E3.1 前需要处理已知技术债：`standing-quad-stretch` 的 `timedDefault` 当前无法完整表达“每侧 25-30 秒”。

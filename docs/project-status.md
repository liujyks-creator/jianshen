...
`Story E4.1: 力量训练执行引擎`、`Story E4.2: 力量训练执行页`、`Story E4.3: 单组完成确认层`、`Story E4.4: 动作替换与跳过`、E4 UI smoke fix、`Story E5.1: 计时训练总结`、`Story E5.2: 力量训练总结`、`Story E5.3: 训练历史与基础趋势`、`Story E5.4: 基础恢复建议`、`Story E6.1: 跟练雏形计划入口`、`Story E6.2: 跟练执行页`、`Story E6.3: 心率抽象状态展示`、`Story E7.1: 训练提醒通知` 和 `Story E7.2: 活跃训练通知边界` 均已通过 Review Gate 并合入 `main`。当前阶段分支 `codex/e7-3-training-preferences` 基于 `main` / `origin/main` 的 `6d0e3d52a96814243e38cbe7c10f5f20a569fb4a` 实施 E7.3，等待 Review Gate。当前无已知 blocker。
...
1. 进入 `Story E7.3: 训练偏好设置` Review Gate。
2. Review Gate 重点确认 DataStore 只保存训练反馈默认偏好，不拥有训练状态机。
3. 确认新建计时/力量计划会消费默认偏好，已保存计划中的显式 `CueSettings` 和 set timer mode 不被静默覆盖。
4. 确认设置页文案没有把计划提醒通知、活跃训练普通 ongoing notification 和训练内倒计时反馈混为一类。
5. 确认 manifest 仍只包含 `POST_NOTIFICATIONS`，未新增 exact alarm、foreground service、健康、传感器、蓝牙或定位权限。

...
E10.10 Plan Persistence、E10.11 Huashu Timer Dial HTML prototype exploration、E10.12 Timer Dial Compose landing、E10.13 Ready Start Gate、E10.14 Rest Extension Semantics And Recording、E10.15 Motion Timing Rules 和 E10.16 Motion Landing 均已基于最新 `main` 推进。当前阶段分支 `codex/e10-16-motion-landing` 将 E10.15 motion token 最小落地到计时训练关键交互：ready gate 到 execution 的局部切换、ready gate / center dial / `+15秒` 的触摸反馈、play/pause glyph 状态切换、Timer Dial marker / ring / center color 状态变化，以及 reduce-motion snap specs。E10.16 不改变 engine time、倒计时、session record、`WorkoutCommand`、`WorkoutEvent`、`+15秒` 二段确认、4 次上限或 rest extension 记录语义；不实现大型页面转场、Stage color picker、声音播放、E12 统计图表、真实心率设备、foreground service、exact alarm、notification action、reset production command、第四套 skin 或 UI 视觉重做；不移动音频，不提交 `.local`、APK、`人工/`、deliverables、截图、日志或 build 输出。
...
15. E10.16 已把 motion token 最小落地到计时训练 ready gate、center dial、Timer Dial 状态变化和 `+15秒` 二段确认反馈；动效仍只消费 UI state / engine state，不改变训练业务语义。
...
5. E12 Stats / Records 可基于 E10.14 的 `timedRestExtensionRecords` 做真实额外休息统计、总统计、图表、平均心率趋势和同日多轮运动分析；不得回填或修改原计划结构。
...
7. E11 手动心率继续保持独立阶段；真实心率设备、Health Connect、Wear OS、BLE 或厂商 SDK 仍需另行决策。

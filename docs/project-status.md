...
5. 后续心率数据源策略和健康数据权限流；E11.1 已完成 source-aware UI/provider boundary，仍不接真实设备、不做手动输入、不持久化心率。E11.2a 已完成 debug-only HUAWEI Band 9 on non-Huawei Android feasibility smoke：当前 Band 9 + 非华为 Android + 华为运动健康可读数据只证明华为运动健康能读取设备，不证明 TrainFlow 能实时读取；用户新版 smoke APK 仍无法发现华为设备，没有 BLE HRS `0x180D` / `0x2A37 notify` 物理证据，因此不进入设备接入。E11.3 已撤销当前执行页局部手动输入，首版不显示、不录入、不统计心率。Apple Watch / HealthKit 保留为未来 iOS 第一优先路线；Health Connect 更适合历史摘要 / 趋势，不作为 E11.2a 实时执行页首选。
...
5. E11.1 Heart-rate source boundary / unavailable state refinement 已完成；E11.2a HUAWEI Band 9 on non-Huawei Android feasibility smoke 已完成；用户新版 smoke APK 仍无法发现华为设备，没有 BLE HRS `0x180D` / `0x2A37 notify` 物理证据，因此不进入 BLE adapter spike 或生产设备接入。后续若继续设备方向，应单独评估 Huawei SDK feasibility；Health Connect 只作为历史摘要 / 趋势候选。E11.3 放弃首版心率显示、录入和统计已实现为当前执行页局部 UI，不持久化、不进入历史趋势。
...
7. E13.1 Sound Cue System 已实现短提示音播放与音频共存；手机扬声器 / 蓝牙耳机 smoke 仍可作为后续设备回归补记录。E13.2 固定女声 cue / 阶段名朗读仍是后续增强，不做用户任意 TTS 或自动语音教练。
...
9. E14.4 Feature UI polish batches 已启动，E14.4-1 训练执行页共性 polish 已通过用户真机重点项确认；E14.4-2 计划编辑 / 计划详情低耦合 polish 已完成第一轮实现。后续仍按 E14.3 审计继续推进剩余计划编辑细节、记录页数据分析和图表 polish、动作库 / 动作详情 polish、跟练雏形 polish。E14.4-2b 阶段内部目标扩展 + TimerDial 外圈时间比例语义仍拆为独立 story；E14.4-2b-1 visual prototype / mock、E14.4-2b-2 data model decision、E14.4-2b-3 model / serializer / editor adapter foundation 与 E14.4-2b-4 editor UI visual/code gate 已进入当前代码基线。不得继续 E14.4-2b-5 engine timeline expansion 或 E14.4-2b-6 TimerDial mapping；下一步只能做 E14.4-2b-4 code review / commit gate。每批 UI 优化继续先做视觉方案 gate，经用户确认后才进入 Kotlin / Compose / 测试实现，实施批次再补测试或 smoke 记录。
...
E11.1 Heart-rate source boundary / unavailable state refinement 已实现并降级为未来底层边界；Android `HeartRateState` 从旧 availability baseline 收口为 `kind + sourceKind` source-aware 状态，可表达无来源、设备已连接但等待读数、设备读数、手动读数、过期读数、provider 不可用和权限不可用。`DisabledHeartRateProvider` 默认输出 no source / unavailable；`MockHeartRateProvider` 可表达设备、手动、过期和不可用抽象状态，但不代表真实设备。E11.3 后生产执行页 UI state 不再携带心率展示 mapper，不显示 `-- bpm` / `未获取心率`，不显示设备或手动读数。后续 Apple Watch / iOS 走 iOS app + watchOS companion + HealthKit / HKWorkoutSession / HKLiveWorkoutBuilder；HUAWEI Band 9 当前只作为非华为 Android feasibility 样本，先验证 BLE HRS `0x180D` / `0x2A37 notify`，不可用时再验证 Huawei Health Kit / Health Service Kit 实时授权读取；Health Connect 更适合未来独立历史摘要 / 趋势。所有未来 adapter 都必须统一输出 TrainFlow `HeartRateState` 并标注来源。E11.1 仍未接真实设备、HealthKit、Huawei、BLE、Health Connect、Wear OS 或厂商 SDK，未实现手动输入 UI，未申请真实健康 / 蓝牙 / 身体传感器权限，未持久化心率，未绘制平均心率趋势，未改变 `WorkoutCommand` / `WorkoutEvent` / 训练引擎语义，也不做医疗判断、危险告警或训练中断依据。E11.2a HUAWEI Band 9 on non-Huawei Android feasibility smoke 已完成；用户新版 smoke APK 仍无法发现华为设备，没有 Band 9 暴露 BLE HRS 的物理证据；E11.3 已撤销首版心率显示、录入和统计。

E11.2a HUAWEI Band 9 on non-Huawei Android feasibility smoke 已实现 debug-only APK 入口并记录；详见 `docs/testing/e11-2a-huawei-band9-feasibility-smoke.md`。本轮验证确认：当前真实条件只证明 Huawei Health 可读 Band 9 数据，不证明 TrainFlow 第三方 App 可实时读取心率；Codex 环境没有可用 ADB / GATT scan 入口，因此曾新增 `app/src/debug` 下的 `Band 9 HRS Smoke` 入口，让用户安装 debug APK 后在非华为 Android 手机上扫描 BLE、连接 GATT、发现 service / characteristic 并尝试订阅 `0x2A37 notify`，屏幕日志用于回传判断。当前仍未取得用户手机上 Band 9 暴露标准 BLE Heart Rate Service `0x180D` 或 Heart Rate Measurement `0x2A37 notify` 的物理证据。用户第二轮截图显示：Huawei Health 连接状态下，`Bonded` 只列出 MERACH / Galaxy Buds Pro 等非目标设备，未显示 HUAWEI / Band 9；这强化了 Band 9 不通过系统 bonded/GATT 路径直接暴露给第三方 App 的风险。官方资料只证明 Band 9 有心率传感器、支持 BLE、Huawei Health 可展示心率；不足以进入 Android BLE HRS adapter spike。当前路线建议是暂不接设备：用户 2026-06-21 反馈新版 smoke APK 仍无法发现华为设备，当前没有 `0x180D` / `0x2A37 notify` 或 bpm notify 证据，不进入 BLE adapter spike；后续若继续设备方向，应单独评估 Huawei SDK feasibility。E11.3 后 MVP 不显示心率、不显示未获取心率占位、不录入心率、不统计心率。Health Connect 只作为未来独立历史摘要 / 趋势候选，Apple HealthKit 继续保留为未来 iOS 路线。E11.3 已移除 debug smoke 入口和 debug BLE 权限；未修改生产 Gradle、main Android Manifest、资源或 Room schema；未持久化心率，未绘制平均心率趋势；手动输入已撤销，不倒灌 E11.2a；未触碰 `.local/verification`。

...
`Story E4.1: 力量训练执行引擎`、`Story E4.2: 力量训练执行页`、`Story E4.3: 单组完成确认层`、`Story E4.4: 动作替换与跳过`、E4 UI smoke fix、`Story E5.1: 计时训练总结`、`Story E5.2: 力量训练总结`、`Story E5.3: 训练历史与基础趋势`、`Story E5.4: 基础恢复建议`、`Story E6.1: 跟练雏形计划入口`、`Story E6.2: 跟练执行页`、`Story E6.3: 心率抽象状态展示`、`Story E7.1: 训练提醒通知`、`Story E7.2: 活跃训练通知边界`、`Story E7.3: 训练偏好设置`、`Story E8.1: Skin contract and registry`、`Story E8.2: Tile Flow 完整视觉重做`、`Story E8.3: Big Type 完整视觉重做`、`Story E8.4: UI skin review checklist`、`Story E9.1: 训练状态恢复与回归测试`、`Story E9.2: 权限与隐私文案` 和 `Story E9.3: MVP 验收清单` 均已通过 Review Gate 并合入 `main`。当前阶段分支 `codex/e9-4-user-test-fix-pack-1` 基于 `main` / `origin/main` 的 `3557a63afd0202a99b92e3c1b46934d0c0c57016` 实施 E9.4：修复计划编辑页数字输入临时清空、计时编辑页立即开始、力量编辑页开始训练，并把历史记录全部 / 按计划 / 按日期清理登记为后续能力。当前仍不实现真实 `WorkoutSession` 持久化、Room repository 闭环、历史记录真实删除、foreground service、后台可靠计时、真实心率设备、语音、完整跟练编排、统一动作选择页或训练模式边界重构。
...
1. 完成 `codex/e9-4-user-test-fix-pack-1` 分支 Review Gate。
2. 若 Review Gate 通过，使用本阶段 debug APK 开始小范围用户测试，并用 `docs/testing/user-test-issue-template.md` 收集问题。
3. 后续进入 E10/E11/E12：训练模式边界重构与统一动作选择页、心率设备策略、数据分析趋势与记录清理能力。

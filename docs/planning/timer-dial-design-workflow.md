...
E14.4-2b-6 TimerDial mapping planning gate 已完成 docs-only source-boundary audit，详见 `docs/testing/e14-4-2b-6-timerdial-mapping-planning-gate.md`。当前生产 TimerDial 仍是 legacy work/rest cycle mapping，v2 outer ring 尚未上线。后续 v2 mapping 应保持既有圆盘 UI、Canvas 几何、内圈、中心圆、暂停 morph、bottom controls、reduce-motion、final countdown、rest extension monotonic progress 和 E14.5 continuous projection identity。内圈继续表达整次训练 total stage progress；12 点数字圆标继续显示 warmup + rounds * stageGroups + between-round rests + cooldown 的 total stage count，target 数不替代该数字。外圈只在当前 stageGroup 内按 1-5 个 targets 的 `plannedDurationSec` 比例分段：1 个 target 为 full ring，2 个 targets 按比例切两段，3-5 个 targets 按各自时长切多段；action / custom / rest target 都参与比例。Warmup、cooldown 和 synthetic between-round rest 不属于 stageGroup targets，外圈使用 single-segment current-stage / legacy-like fallback。`+15s` 不重算 planned ratio、不插入 target、不增加第 6 段、不改 snapshot，active rest target 或 synthetic rest 的中心倒计时可延长但外圈 / 内圈 progress 不得倒退。后续拆分为 6a mapping model/state tests、6b production mapper、6c smoke / visual QA；不得直接从 planning gate 修改 production TimerDial mapping。

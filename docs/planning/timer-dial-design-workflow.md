...
E14.4-2b-3 / E14.4-2b-4 的旧本地实现未通过 review gate，已 rolled back / not accepted；restart 版已重新落地并推送：E14.4-2b-3 只接受 model / serializer / editor adapter foundation，E14.4-2b-4 只接受 editor UI visual/code gate。当时 v2 editor / plan detail 以 `待执行映射完成后可开始` 禁用开始训练，后续已由 E14.4-2b-5b-3 替换为 adapter-expandable / fail-closed start gate。E14.4-2b-6a / 6b / 6c 已按 test-first split 完成 production TimerDial mapping 和 smoke / visual QA：生产 TimerDial 读取 adapter-derived composition v2 metadata，只把外圈数据源切到当前 stage group targets 的 planned duration ratio，并保持当前 TimerDial UI、暂停 morph、continuous projection、reduce-motion、final countdown 和 rest extension monotonic progress 边界。
...
E14.4-2b-6 TimerDial mapping 链路已完成：planning gate 见 `docs/testing/e14-4-2b-6-timerdial-mapping-planning-gate.md`，model/state tests 见 `docs/testing/e14-4-2b-6a-timerdial-mapping-model-state-tests.md`，production mapping 见 `docs/testing/e14-4-2b-6b-timerdial-production-mapping.md`，smoke / visual QA review 见 `docs/testing/e14-4-2b-6c-timerdial-mapping-smoke-visual-qa.md`。Production v2 mapping 保持既有圆盘 UI、Canvas 几何、内圈、中心圆、暂停 morph、bottom controls、reduce-motion、final countdown、rest extension monotonic progress 和 E14.5 continuous projection identity。内圈继续表达整次训练 total stage progress；12 点数字圆标继续显示 warmup + rounds * stageGroups + between-round rests + cooldown 的 total stage count，target 数不替代该数字。外圈只在当前 stageGroup 内按 1-5 个 targets 的 `plannedDurationSec` 比例分段：1 个 target 为 full ring，2 个 targets 按比例切两段，3-5 个 targets 按各自时长切多段；action / custom / rest target 都参与比例。Warmup、cooldown 和 synthetic between-round rest 不属于 stageGroup targets，外圈使用 single-segment current-stage / legacy-like fallback。`+15s` 不重算 planned ratio、不插入 target、不增加第 6 段、不改 snapshot，active rest target 或 synthetic rest 的中心倒计时可延长但外圈 / 内圈 progress 不得倒退。
...
E14.6-3 planning 结论记录在 `docs/testing/e14-6-3-stage-style-icon-planning.md`，E14.6-3a data contract / model decision 记录在 `docs/testing/e14-6-3a-stage-style-data-contract-decision.md`。TimerDial 消费 style 时保持 E14.4-2b-6 mapping：外圈仍按当前 stage group targets 的 planned duration ratio 分段；内圈仍表达整次训练总阶段进度；12 点数字圆标仍表达 warmup + rounds * stageGroups + between-round rests + cooldown。中心圆 icon 使用 active target `iconKey`，缺失时回退到 stage group，再回退到 `warmupStyle` / `cooldownStyle` / `restBetweenRoundsStyle` 或阶段类型默认 icon；外圈 / 中心颜色使用 target -> stageGroup -> boundary style -> type default fallback。Warmup、cooldown 和 synthetic between-round rest 即使不是 targets，也应在视觉上拥有自己的默认颜色和 icon。`+15s` 不插入 target、不增加第 6 段、不改 snapshot、不改变 rest-extension record。

内部阶段圆环下的浅色承托圆环可以在未来 visual polish 中单独加粗，但不应和 E14.6-1 progress rebound fix 或 E14.6-3 style data planning 混为同一实现。E14.6-3a 不建议 Room migration；boundary style 只进入 versioned timed composition JSON payload 和 session snapshot JSON。E14.6 planning gate 不写 Kotlin / Compose / Room / tests，不生成 APK，不启动 AVD。

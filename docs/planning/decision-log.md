...
| D-038 | Accepted | E10.9 用户测试反馈拆为独立后续 story，不混入已完成 polish。 | Timer Dial 视觉减字、总剩余时间放大居中、圆盘放大、环线层级、宽底层圆环、动态浅点和中心圆简化进入 E10.9 Review Fix / User Test Fix，并保留 continuous progress、pause freeze、terminal freeze、rest extension monotonic progress。计划保存真实持久化和保存入口 audit 进入 E10.10；`huashu-design` HTML 高保真原型进入 E10.11；`countdown_beep1.mp3`、`.local/audio/stage_bell_copper_clean.mp3`、蓝牙 / 扬声器 smoke 和不 duck / 不抢占外部音频进入 E13；总统计、图表、平均心率趋势和同日多轮运动分析进入 E12。本轮 docs-only，不提交 `.local`、APK、`人工/`、build 输出或音频资源。 |
...
| D-045 | Accepted | E11.1 第一切片是 `Heart-rate source boundary / unavailable state refinement`。 | E11.1 只收口 source-aware `HeartRateState` / `HeartRateProvider` 边界和不可用状态表达，让执行页能区分无来源、设备已连接但无读数、设备读数、手动读数、过期读数、provider 不可用和权限不可用。E11.1 不接真实设备、Health Connect、Wear OS、BLE、HealthKit、Huawei Health Kit / Health Service Kit 或厂商 SDK，不实现手动输入 UI，不申请真实健康、蓝牙或身体传感器权限，不持久化心率，不绘制平均心率趋势，不做医疗判断、危险告警或训练中断依据。手动输入保留为后续可选补充，不是必做项，也不得伪装成设备数据。 |
...
| O-006 | Accepted | 后续健康数据与可穿戴设备的接入策略是什么？ | 已按 D-042 / D-045 / D-046 收敛：心率来源以设备获取为优先，手动输入只是可选补充；两者都没有时显示未获取心率。E11.1 已完成 source-aware provider boundary 和不可用状态细化，不接真实设备、不做手动输入、不持久化心率。E11.2a 下一步先用当前 HUAWEI Band 9 + 非华为 Android + 华为运动健康可读数据的真实条件做 feasibility smoke，验证 BLE HRS `0x180D` / `0x2A37 notify`，再决定 BLE HRS adapter spike、Huawei Health Kit / Health Service Kit feasibility、历史同步或暂不接设备路线。Apple Watch / iOS 仍走 HealthKit + workout session adapter，并保留 watchOS companion 路线；Health Connect 更适合历史摘要 / 趋势，不作为实时执行页首选。所有 adapter 都必须统一输出 TrainFlow `HeartRateState`，标注来源，且不能让 SDK model 泄漏到 UI、历史或训练引擎。 |

...
| D-050 | Accepted | E14.4-2 采用方案 B，并把计时目标编排 + TimerDial 圆环语义拆成独立 story。 | E14.4-2 后续 implementation 只做低耦合计划编辑 / 详情 polish：计划详情采用可折叠计划播放列表，当前计划的 `开始训练`、编辑、复制和 `删除当前计划` 都归属展开计划卡片；计划颜色是用户手动设置的计划色，不从首个阶段或目标组推断；编辑页 sticky action 以绿色 `保存计划` 为主按钮、深色实心 `开始训练` 为次按钮；颜色选择复用推荐色 / 更多颜色大色板；力量目标组默认折叠，展开后编辑重量、次数、休息和颜色；用户界面清理 `WorkoutPlan`、`strength block`、`manual_start`、`planned set`、`interval stage` 和 `rest step` 等工程化文案。计时训练两层结构（外层目标编排 + 内层阶段）、外层 / 内层拖动和设色、外层总时长由内层阶段求和、TimerDial 外圈按内部阶段时长占比分段，必须进入 E14.4-2b 独立 planning / visual gate，不得在 E14.4-2 中静默改 Room schema、训练引擎、`WorkoutCommand`、`WorkoutEvent`、session record、声音语义或统计比较 key。 |

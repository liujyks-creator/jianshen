...
| O-006 | Accepted | 后续健康数据与可穿戴设备的接入策略是什么？ | 已按 D-042 / D-045 收敛：心率来源以设备获取为优先，手动输入只是可选补充；两者都没有时显示未获取心率。E11.1 先做 source-aware provider boundary 和不可用状态细化，不接真实设备、不做手动输入、不持久化心率。后续 Apple Watch / iOS 走 HealthKit + workout session adapter；Huawei band / Huawei Health 先做 feasibility，可能路径是 Huawei Health Kit / Health Service Kit adapter，或 iOS 上读取 Apple Health 中来自 Huawei Health 的 source data；通用心率设备可评估标准 BLE Heart Rate Service adapter。所有 adapter 都必须统一输出 TrainFlow `HeartRateState`，标注来源，且不能让 SDK model 泄漏到 UI、历史或训练引擎。 |

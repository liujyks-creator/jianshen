...
| D-045 | Accepted | E11.1 第一切片是 `Heart-rate source boundary / unavailable state refinement`。 | E11.1 只收口 source-aware `HeartRateState` / `HeartRateProvider` 边界和不可用状态表达，让执行页能区分无来源、设备已连接但无读数、设备读数、手动读数、过期读数、provider 不可用和权限不可用。E11.1 不接真实设备、Health Connect、Wear OS、BLE 或厂商 SDK，不实现手动输入 UI，不申请真实健康、蓝牙或身体传感器权限，不持久化心率，不绘制平均心率趋势，不做医疗判断、危险告警或训练中断依据。手动输入保留为后续可选补充，不是必做项，也不得伪装成设备数据。 |
...
| O-006 | Accepted | 后续健康数据与可穿戴设备的接入策略是什么？ | 已按 D-042 / D-045 收敛：心率来源以设备获取为优先，手动输入只是可选补充；两者都没有时显示未获取心率。E11.1 先做 source-aware provider boundary 和不可用状态细化，不接真实设备、不做手动输入、不持久化心率；Health Connect、Wear OS、BLE 或厂商 SDK 的具体权限、来源、设备范围和失败状态仍需在 E11.2 或独立设备 story 中细化。 |

...
| D-052 | Accepted | E14.4-2b-3 / E14.4-2b-4 本地实现未通过 review gate，已回滚且不作为当前实现基线。 | D-051 的 versioned two-layer timed composition 仍作为规划决策保留，但本地 `TimedCircuitBlock.composition` Kotlin carrier、serializer / adapter 实现、COMPOSITION_V2 editor-only UI 和 `待执行映射` 生产入口均未被接受。当前基线没有已接受的 composition v2 Kotlin / Compose 实现；Room version 仍为 4，不新增 table / column / relation / migration。未来若重启 E14.4-2b-3 / E14.4-2b-4，必须从 clean template-based dev story 开始，重新实现、测试和 review，不得直接衔接 E14.4-2b-5 / E14.4-2b-6。 |
| D-053 | Accepted | E14.4-2b 后续必须先完成流程 reset / review gate，且 UI / APK story 默认使用本地虚拟测试环境。 | E14.4-2b-1 到 E14.4-2b-4 暴露出提示词模板、技能读取、视觉基准和 Android smoke 纪律漂移。后续不得直接进入 E14.4-2b-5 / E14.4-2b-6；必须先按 `DEV_STORY_PROMPT_TEMPLATE.md` / `CODE_REVIEW_PROMPT_TEMPLATE.md` 重新启动 template-based dev story、确认 visual gate、运行 Gradle 验证，并用 `C:/Users/25073/Desktop/jianshen/.local/android-sdk`、默认 AVD `TrainFlow_Pixel_API_36` 尝试 adb smoke。Android UI / APK / 真机截图修复类任务的截图、UI tree 和 logcat 只能写入 `.local/smoke/<Story ID>/`，不得写入 `.local/verification` 或提交 `.local/`。 |
...
| O-008 | Accepted | E14.4-2b 是否正式引入明确的阶段内部目标 composition model？ | 已按 D-051 收敛：正式采用 versioned two-layer timed composition 作为长期数据方向，优先作为 `WorkoutPlan.blocks` JSON / `WorkoutSession.planSnapshot` JSON payload，不改 Room table shape；旧计划通过 compatibility wrapper 显示和执行，查看不写回，用户明确保存 / 转换后才写入 composition v2；`轮间休息` 继续作为顶层 round configuration，执行时展开为轮间休息 step；12 点数字圆标继续沿用整次执行 timeline 的总阶段数语义；E12 trend key 必须纳入 composition version、stage id、target id 和结构签名。 |

...
| D-052 | Accepted | E14.4-2b-3 restart 仅接受 model / serializer / editor adapter foundation，E14.4-2b-4 仍未接受。 | 旧 E14.4-2b-3 / E14.4-2b-4 本地实现未通过 review gate，已回滚且不作为当前实现基线。本轮 E14.4-2b-3 restart 从已提交规划重新实现：新增 versioned timed composition v2 Kotlin payload、stage group / target / compatibility metadata、归一化规则、现有 `WorkoutPlan.blocks` JSON 与 `WorkoutSession.planSnapshot` JSON round-trip、legacy timed JSON 兼容、legacy wrapper / editor draft adapter，以及显式 export / conversion 才输出 composition v2 的边界。Room version 仍为 4，不新增 table / column / relation / migration；当前仍没有 v2 Compose editor UI、执行 timeline mapping、TimerDial production mapping、session record 语义变更、声音语义变更或心率 UI 回归。 |
| D-053 | Accepted | E14.4-2b 后续不得直接进入 E14.4-2b-5 / E14.4-2b-6，下一步必须是 E14.4-2b-4 editor UI visual/code gate。 | E14.4-2b-1 到旧 E14.4-2b-4 暴露出提示词模板、技能读取、视觉基准和 Android smoke 纪律漂移。E14.4-2b-3 restart 已完成基础层，但不授权直接进入 engine / TimerDial。下一步必须先按 `DEV_STORY_PROMPT_TEMPLATE.md` / `CODE_REVIEW_PROMPT_TEMPLATE.md` 重新启动 E14.4-2b-4 editor UI visual/code gate、确认 visual gate、运行 Gradle 验证；若涉及 Android UI / APK / 真机截图修复类任务，再用 `C:/Users/25073/Desktop/jianshen/.local/android-sdk`、默认 AVD `TrainFlow_Pixel_API_36` 做 smoke。截图、UI tree 和 logcat 只能写入 `.local/smoke/<Story ID>/`，不得写入 `.local/verification` 或提交 `.local/`。 |
...
| O-008 | Accepted | E14.4-2b 是否正式引入明确的阶段内部目标 composition model？ | 已按 D-051 / D-052 收敛：正式采用 versioned two-layer timed composition 作为长期数据方向，优先作为 `WorkoutPlan.blocks` JSON / `WorkoutSession.planSnapshot` JSON payload，不改 Room table shape；E14.4-2b-3 restart 已落地 model / serializer / adapter foundation。旧计划通过 compatibility wrapper 显示和执行，查看不写回，显式 export / conversion 后才写入 composition v2；`轮间休息` 继续作为顶层 round configuration，未来执行时展开为轮间休息 step；12 点数字圆标继续沿用整次执行 timeline 的总阶段数语义；E12 trend key 必须纳入 composition version、stage id、target id 和结构签名。 |

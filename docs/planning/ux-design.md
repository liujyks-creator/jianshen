...
## E18 实时记录与结束后入口（E18-CC-P01）

用户已接受正常训练持续实时保存、正常结束确定真实截止点、停止后不续录/补造、重开只按已落盘事实处理。来源 V2 F.69.2/P01–P08（757211 bytes，SHA256=`EC37A27512A540905D4DCF8B4588151718EECBBD275C1A4D09736B7F9B4A6112`）；当前条款与 D5 窄替代见 [正式计划](e18-onward-epic-story-plan.md#current-status) F.8/F.9/F.11/F.12。原训练 UI、关键操作/停止确认、历史/非 HR 趋势和独立恢复建议保持；不新增低空间预检、存储故障向导或同进程写恢复/重试入口。该收窄不改变设备重连政策。文档候选待独立审查，不代表 S05 PASS、实现 READY 或后续 Writer 解锁。
...
- 实时心率不再走旧的页面内联展示位；E16 之后的方向是 App 内可拖动浮动心率胶囊。偏好开启后跨 TrainFlow App 页面显示，但必须避让训练主任务和固定按钮。
...
> E17-2 当前合同：心率默认关闭；用户在训练偏好显式开启后，心率是重要训练能力，胶囊可在 TrainFlow 前台跨页面显示。未训练只显示不记录；训练中记录、复盘和导出为后续独立能力。旧页面内联心率位、手动输入和旧平均心率占位不恢复。

胶囊视觉、信息布局、collapsed / expanded、拖动、吸附、安全区和动画继续按 D-079 `adopted / frozen / direct reuse`。E17-2 只更新 runtime 状态与显示时机：
...
- 动作短提示。
- 心率浮动胶囊可在偏好开启时显示，但不得遮挡 `完成本组`、暂停 / 结束、目标重量 / 次数等主信息。
...
训练完成与保存确认分别表达：原 engine completed/user_abandoned 冻结真实截止点和执行结果，S04 终态事务提交且完整读回才为 Saved。原正常返回、回训练主页和恢复建议入口可在 Saved 后使用，不等待 binding 清理/Released；cleanup 未定仍禁止新冲突写，不把它改称保存失败。终态未确认则诚实显示保存失败/未确认，不承诺所有点已保存，不新加重试恢复页面。真实 clear 只停接新输入/新写，已在途事务原子完成，不另补终结。
...
原已实现力量保存与计划/实际值呈现继续保留；未来迁移本模式时，同一 session 的 Start 和终态消费者必须一起切换，并沿 §11.1 分开训练完成、Saved 与 cleanup。不能以本轮文档宣称力量已接入新链。
...
已 Saved 的本场可从原结束后/历史入口读取，不以 Released 作为可读门禁。重开只展示/处理已落盘事实：已有 terminal 使用原结果，未终态沿原 CS-04B 的 durable 前缀处理，不续跑、不以重开时间补实际结束、不补造未保存样本。当前计划改变不得污染原记录名称、实际值和关联；已有非 HR 趋势及未知日期分层保持。
...
该恢复建议仍是原独立能力，沿本次 session 与已保存实际训练数据的原关联进入；已 Saved 的正常导航不等待 binding 清理。这里的“恢复”不是故障后续录或重建训练，不新增自动修复/补造数据职责。
...
- 计时训练自动推进偏好。
- 力量训练本组计时默认模式。
- 心率显示开关、设备状态入口、用户年龄 / 最大心率参数和上限提示阈值。心率默认关闭，canonical 入口是 `设置 -> 训练偏好 -> 心率与设备`；设置说明展示用途、权限、local-first 隐私、非医疗边界，以及 Band 9 心率广播与 Huawei Health 当前互斥。权限只在用户主动扫描 / 连接时请求，不在开启开关或 App 启动时请求。设备选择只保存 identifier / display name 便利提示；用户点击“连接已保存设备”后才做一次有限时精确匹配。关闭后不扫描、不连接、不继续采集，保留清除已保存设备入口。
...
5. 临近结束倒计时动画是否明显但不烦。
6. 心率浮动胶囊是否可见、可拖动、可贴边，并且不遮挡主任务或造成底层按钮误触。
7. 训练中打开动作详情后是否能无损返回。
...
2. 心率胶囊视觉与互动已冻结直接复用，不再做 HTML、颜色、尺寸、布局、动画或交互变体。未来单次训练心率复盘参考用户提供的划船机统计 / 图表效果，在数据能力成立后另做独立视觉审查与用户确认；截图不复制或提交。

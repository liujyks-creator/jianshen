...
> E11.3 更新：首版已撤销心率显示、录入和统计。E16 之后确认 BLE HRS 路线可行，但不恢复旧的页面内联心率位、未获取占位、手动输入或平均心率趋势。

当前 UI 方向是 App 内可拖动浮动心率胶囊：

| 状态 | 表现建议 |
|---|---|
| 未启用 / 未连接源 | 灰色胶囊，短文案，不占主布局 |
| 权限未赋予 | 灰色或琥珀色胶囊，点击展开权限说明入口 |
| 蓝牙关闭 | 灰色或琥珀色胶囊，点击展开蓝牙状态 |
| 正在连接 / 等待数据 | 中性胶囊，显示连接中或等待数据 |
| live bpm + 区间 | 显示 `热身 105 bpm`、`燃脂 122 bpm` 等，整体颜色按区间变化 |
| 数据过期 / 离线 | 弱化上次状态或显示离线，不伪装为当前 bpm |
| 超过上限 | 深红视觉提示，仅表示超过用户设置上限，不做医疗告警 |
...
- 计时训练自动推进偏好。
- 力量训练本组计时默认模式。
- 心率显示开关、设备状态入口、用户年龄设置和上限提示阈值预留。心率默认关闭，canonical 入口是 `设置 -> 训练偏好 -> 心率`；开启前必须展示用途、BLE 权限、隐私和非医疗说明。偏好开启后浮动胶囊在 App 内显示；无训练时只显示不记录，训练中按后续数据契约记录。设备选择只保存 identifier / display name；关闭后不扫描、不连接、不记录，并提供清除已保存设备入口。
...
2. 心率浮动胶囊的最终 HTML 高保真方案、默认落点、拖动吸附和安全区域细节仍需 E16-3a 验证；产品方向已确定为偏好开启后显示。

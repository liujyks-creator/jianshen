...
- active / paused training 的 Activity / Route `ON_STOP` 本身不是 cleanup。连接未 cleanup、未丢失时，回前台必须仍是同一 Application owner、同一 attempt lineage 与 current bpm；后台意外断连时，只允许同一 owner 在合法 FGS 下以新 generation / attempt 恢复 exact target，不能伪称 same attempt。
- 显式断开、opt-out、清除 target、permission / Bluetooth loss、training terminal 或 FGS legality failure 走各自 typed stop reason。显式断开持久化 suppression 并保留 opt-in、saved target 与个人参数；只有明确重新连接或选择目标解除。

...
- 2026-07-28 实机反馈及后续 option 3 `Soft Zone Halo` 选择对 D-079 / D-082 作一次 collapsed-only 窄 supersession：collapsed 胶囊使用 `widthIn(min = 116dp, max = 180dp)` 的 intrinsic centered Row，不再让 Text 填满可用宽度；保留 `13dp / 9dp` padding，恢复既有 status / zone tone 的 `8dp` 圆点与精确 `8dp` dot-to-label gap，短文案自然约 `125–145dp`，极短文案使用 `116dp` minimum，长文案保持单行并在 `180dp` 内安全省略。三个个人参数共用一个 `10dp` 外框，每行最小 `64dp`、padding `12dp / 6dp`、`20dp` 官方 Material 图标、`16dp` 图标到标题列间距、约 `12sp / 16sp` 副标题、`60dp × 40dp` / `10dp` 圆角 / `8dp` 内边距值框、值框后 `8dp` 间距与 `20dp` auto-mirrored chevron，并用显式 `1dp` divider 分隔。年龄 / 个人最大心率 / 上限提醒精确使用 Google 官方 outlined person / monitor-heart / notifications-none，尾部使用官方 chevron-right；四个 vector 从 `google/material-design-icons`（Apache-2.0）窄量 vendoring，保留官方 viewport/path 数据并由 Compose theme/content tint，不新增 broad icon dependency。有明确区间或超过上限时，继续移除左侧强调条，使用对应既有区间色约 `16%` 的轻微着色 surface、组件局部柔和 halo / gradient、明确区间文字，以及按实际底色选择的高对比前景。不得扩展为页面级渐变、装饰光斑、饱和区间色 + 通用白字。2026-08-07 后续用户验收只对“geometry 零变化”再作一项更窄 supersession：size、viewport 或 exclusion policy 触发重新布局时，必须保留已存储的 `HeartRateCapsuleSnapEdge`，继续贴在原 LEFT / RIGHT 安全边；只有 pointer drag release 仍按 release point 做 nearest-edge inference 并更新存储边。expanded leading dot / label、常规 `276dp × max 214dp`、compact `252dp × max 190dp`、expanded padding、四格信息、圆角、信息层级、capsule dimensions、vertical clamp、safe-area / exclusion policy、movement threshold、拖动手势、展开 / 收起、motion、visual / content / state、IME 和其他 geometry 全部继续冻结。无区间的连接 / 数据状态仍必须显示 tone dot，并继续使用中性或弱提示表现。

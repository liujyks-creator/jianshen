...
- 2026-07-28 实机反馈及后续 option 3 `Soft Zone Halo` 选择对 D-079 的颜色冻结作一次窄 supersession：三个个人参数改用紧凑 setting row；collapsed 短文案在胶囊内视觉居中，长文案保持单行并安全省略。有明确区间或超过上限时，移除左侧强调条，使用对应既有区间色约 `16%` 的轻微着色 surface、组件局部柔和 halo / gradient、明确区间文字，以及按实际底色选择的高对比前景。不得扩展为页面级渐变、装饰光斑、饱和区间色 + 通用白字，也不得借此改变胶囊尺寸、圆角、信息层级、拖动 / 吸附、展开 / 收起、motion、viewport、安全区或 IME 行为；`HeartRateCapsuleGeometry.kt` 保持零变化。无区间的连接 / 数据状态继续使用中性或弱提示表现。

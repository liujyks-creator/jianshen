...
- 2026-07-28 实机反馈对 D-079 的颜色冻结作一次窄 supersession：有明确区间或超过上限时，胶囊使用对应既有区间色约 `16%` 的柔和底色、同色实心侧边强调条、明确区间文字，以及按实际底色选择的高对比前景。不得改成饱和区间色 + 通用白字，也不得借此改变胶囊尺寸、圆角、信息层级、拖动 / 吸附、展开 / 收起、motion、viewport、安全区或 IME 行为；`HeartRateCapsuleGeometry.kt` 保持零变化。无区间的连接 / 数据状态继续使用中性或弱提示表现。
